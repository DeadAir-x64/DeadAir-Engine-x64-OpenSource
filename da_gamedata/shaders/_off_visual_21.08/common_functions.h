...
// [DA_PORT] DEPRECATED: use SRGBToLinear / LinearToSRGB from srgb.h instead.
// Kept for backward compatibility with any shader still calling these.
float3 da_srgb_to_linear( float3 c ) { return SRGBToLinear(c); }
float3 da_linear_to_srgb( float3 c ) { return LinearToSRGB(c); }

// [DA_PORT] Filmic tone mapping operators.
// da_tonemap_params.x now selects: 0 = Reinhard (vanilla), 1 = ACES-fit, 2 = Hable (filmic)
// da_tonemap_params.y = white point (default 1.7 for Reinhard, ~11.2 for filmic)
// Renamed from da_tonemap to avoid collision with da_tonemap() helper in da_taa.ps.
uniform float4 da_tonemap_params;

// ACES fit (Krzysztof Narkowicz, 2016) — cheap filmic curve.
float3 da_tonemap_aces( float3 x )
{
	const float a = 2.51h, b = 0.03h, c = 2.43h, d = 0.59h, e = 0.14h;
	return saturate( (x*(a*x+b)) / (x*(c*x+d)+e) );
}

// Hable filmic (John Hable, Naughty Dog / Uncharted 2) — more controllable filmic curve.
float3 da_tonemap_hable( float3 x )
{
	const float A = 0.15h, B = 0.50h, C = 0.10h;
	const float D = 0.20h, E = 0.02h, F = 0.30h;
	return ((x * (A * x + C * B) + D * E) / (x * (A * x + B) + D * F)) - E / F;
}

// Normalize Hable to keep mid-tones at reasonable brightness.
float3 da_tonemap_hable_normalized( float3 x, float whitePoint )
{
	float3 toned = da_tonemap_hable(x);
	float3 white = da_tonemap_hable(whitePoint.xxx);
	return toned / white;
}

// [DA_PORT] ⚠️ Цветокоррекция (ASC CDL) здесь НАМЕРЕННО не применяется, хотя «линейное
// пространство до тонемаппинга» — каноническое место по ACES. Причины:
//
// 1. Грейдинг уже применяется в postprocess.ps (da_apply_cdl), в гамма-пространстве, и
//    применять одни и те же величины ДВАЖДЫ — ошибка: эффект удваивается, а сдвиг и степень
//    некоммутативны между пространствами. Точка применения должна быть ОДНА (RED IPP2:
//    grading — отдельная стадия конвейера, а не размазанная по нему).
// 2. Все профили (da_grade_profiles в xrRender_console.cpp) подобраны ГЛАЗАМИ под гамма-
//    пространственное применение. Сдвиг (offset) в линейном пространстве поднимает ВСЮ
//    кривую равномерно — +0.03 по синему при линейных полутонах ~0.18 даёт подъём канала
//    на ~17%, тогда как в гамме тот же номинал ощущается в разы слабее. Отсюда «всё
//    слишком синее» на профиле «бирюза и охра»: двойное применение + усиленный сдвиг.
// 3. Переводить профили на линейное применение — значит перебирать всю таблицу заново,
//    глядя в игру; вслепую это гадание.
//
// Единственное место грейдинга — postprocess.ps. Биндеры da_grade_* в r2.cpp оставлены:
// RegisterConstantSetup для константы, которой нет в шейдере, безвреден.

void tonemap( out float4 low, out float4 high, float3 rgb, float scale)
{
	rgb = rgb * scale;

	const float fWhiteIntensity = da_tonemap_params.y;
	const float fWhiteIntensitySQR = fWhiteIntensity * fWhiteIntensity;

	float4 lowReinhard = ((rgb * (1 + rgb / fWhiteIntensitySQR)) / (rgb + 1)).xyzz;
	float4 lowACES = da_tonemap_aces(rgb).xyzz;
	float4 lowHable = saturate(da_tonemap_hable_normalized(rgb, fWhiteIntensity)).xyzz;

	// da_tonemap_params.x выбирает оператор: 0 = Reinhard, 1 = ACES, 2 = Hable
	// Плавное смешивание, чтобы переключение не давало рывка.
	if (da_tonemap_params.x > 1.5h)
		low = lowHable;
	else if (da_tonemap_params.x > 0.5h)
		low = lerp(lowReinhard, lowACES, saturate(da_tonemap_params.x));
	else
		low = lowReinhard;
