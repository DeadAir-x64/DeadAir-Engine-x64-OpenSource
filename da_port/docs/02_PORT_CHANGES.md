...
(инфраструктурные вещи — просто `DA:`). Ниже — полный список: **1118 правк(и) в 230 файлах**.
...
- **:269** — WaitCalcTask(); // [DA_PORT] см. WaitCalcTask: ниже освобождается то, что задача читает
- **:445** — Проверка на пустоту обязательна: задачу теперь снимают и в WaitCalcTask, поэтому
- **:458** — Сильный ветер: смешиваем исходное качание с модовским, доля задаётся r__grass_sway.
