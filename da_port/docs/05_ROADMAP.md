...
**✅ Ожидание закрыто (25.08).** Это была нехватка контекстов отрисовки: 122 обхода на 4 контекста.
`R__NUM_AUX_CONTEXTS` 1 → 5 (`xrEngine/Engine.h`), оба пула масштабируются от макроса сами. Замер на
одном месте, 100 кадров на конфигурацию: работа кадра 5.27 → 5.02, render 3.93 → 3.75, фаза света
1.58 → 1.40, ожидание списков 0.38 → 0.21, волн 31 → 14.

⚠️ Макрос задаёт размер `marker[R__NUM_CONTEXTS]` в `vis_data` (`xrEngine/vis_common.h`), а эта
структура лежит внутри КАЖДОГО визуала и ходит между библиотеками рендера и игры ⇒ нужна полная
пересборка и выкладка ВСЕХ библиотек, включая `xrEngine.exe` (сборка делает `xr_3da.exe`, копию надо
снимать руками — см. `13_RELEASE_MAP.md`).

⭐ Приём, который стоит повторять: чтобы не пересобирать дважды ради сравнения, добавлена ручка
`r__max_parallel_ctx`, ограничивающая РАЗДАЧУ контекстов при том же скомпилированном числе. Сравнение
стало мгновенным и в одной сборке — и именно поэтому вышло чистым.

### ✅ Накопление света: половину давала СМЕНА ПРОХОДА (25.08)

Разбор `da_accum_prof N` (семь зон внутри `accum_spot` + полное время `accum_point`) опроверг обе
рабочие версии: матрицы теней 7%, поиск констант по строковым именам 8%. Половина цены —
`set_Element`, 1.06 мкс за вызов, по два на лампу, ~150 ламп за кадр.

Отсечка «та же таблица констант — выходим» не срабатывала ни разу: накопление ходит через кадр —
маска, лампа, маска, лампа. ⛔ Слить проходы в пачки нельзя: маска пишет номер лампы в трафарет, а
следующий проход тут же его читает; маска соседней лампы затрёт чужую область.

Внутри `set_Constants` самой дорогой частью оказалась перетасовка набора буферов (0.77 мс на кадр из
1.98) — обход ШЕСТИ стадий конвейера по 14 слотов на каждую смену прохода, при том что в деле обычно
две. Правка: снимок сырыми указателями вместо считающих ссылки + пропуск стадий, ни разу не
получавших буфер (`r__cb_stage_skip`, бит ставится при первой раскладке и не снимается).

| 150 кадров на конфигурацию | 0 | 1 |
|---|---|---|
| `set_Element` в накоплении | 0.138 | **0.090** |
| накопление конусов / точечных | 0.353 / 0.154 | 0.307 / 0.132 |
| **render (медиана)** | 3.490 | **3.360** |
| работа кадра | 4.780 | 4.610 |

Выигрыш (0.13) шире самого накопления (0.07): правка общая для всех ~460 проходов кадра.

**Следующая цель в этом же месте:** обработчики констант — 0.73 мс на кадр. `set_Constants` в конце
обходит ВСЮ таблицу констант шейдера и зовёт `handler->setup` у каждой, у которой обработчик есть.

⛔ Прибор `da_accum_prof` нельзя включать одновременно с `r__graph_prof`: два таймера на одном пути,
все зоны вырастают вдвое.
