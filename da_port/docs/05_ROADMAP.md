...
### 🔥 Выборка вершин из шейдера (vertex pulling) — потолок измерен, работа начата

Единственное направление, где после дня замеров остался **кратный**, а не процентный запас. Всё
ниже — измерено в игре (`da_geom_dump`, `da_frame`), не предположено.

**Что мерили и что вышло.** Кадр: ~1900 вызовов отрисовки, из них статика 1476, разложенные по
**313 проходам** — при том что разных пиксельных шейдеров всего **20**. Разница в 293 прохода это
не разная геометрия, а **разные наборы текстур на одном и том же шейдере**:

```
deffer_base_lmh_steep_db-hq:  150 проходов на 303 объекта   (2 объекта на проход)
deffer_base_bump-hq:           12 проходов на  12 объектов   (1 объект на проход)
```

**Почему это не тупик, в который мы упирались весь день.** Дешёвая склейка (`1476 -> 1310`) требует,
чтобы диапазоны индексов шли ВСТЫК — замер показал заполнение 15%, чужая геометрия вперемешку, тема
закрыта. Vertex pulling это требование снимает вовсе: вершины достаются по индексу из общего буфера
(`SV_VertexID`), соседство не нужно. Номер объекта берётся из `SV_InstanceID` — это стандартная
замена отсутствующему в DX11 `SV_DrawID`, один `DrawInstanced(maxVerts, numObjects)` на пачку.

**Этапы (первый не трогает ассеты вообще):**

| этап | что делаем | вызовов статики | ассеты |
|---|---|---|---|
| сейчас | — | 1476 | — |
| **1** | pulling + слияние ВНУТРИ прохода | **~313** | не трогаем |
| 2 | текстурные массивы (слияние между проходами) | ~20-80 | переупаковка текстур |

**Разведка, снятая приборами на загрузке уровня (`r2_loader.cpp`, диагностика оставлена):**

- буферов вершин **39**, разных форматов **3**, и все три ровно по **32 байта** — вся геометрия
  уровня влезает в один буфер с общим шагом, тридцати путей нет;
- ⭐ первые 24 байта у всех трёх форматов ИДЕНТИЧНЫ, различаются только последние 8:

```
          0-11            12-15    16-19     20-23      24-27        28-31
формат 0  POSITION float3 NORMAL   TANGENT   BINORMAL   TEXCOORD0 short4 (8 байт)
формат 1  POSITION float3 NORMAL   TANGENT   BINORMAL   COLOR0       TEXCOORD0 short2
формат 2  POSITION float3 NORMAL   TANGENT   BINORMAL   TEXCOORD0    TEXCOORD1 (lmh)
```

- урезанный (теневой) поток: 17 буферов, 1 формат, 12 байт — только позиция;
- ⭐ у статики **мировая матрица единичная** (`set_xform_world(Fidentity)` ставится ОДИН раз на
  проход, координаты запечены в вершины) — значит на объект НЕ нужны свои матрицы, только «где
  лежит его геометрия»: три числа (смещение вершин, смещение индексов, число индексов). Это на
  порядок проще общего случая из статей, где на каждый объект едет своя матрица мира.

**⚠️ Главный риск — молчаливый.** Pulling рисует БЕЗ привязки вершинного буфера и разметки, а у нас
уже записаны два случая, когда именно это ломалось без единого сообщения: отброс вызова при
«формат вершины ≠ VS» и осевший навсегда ноль в кэше разметки (`[[silent-draw-drop]]`,
`[[invisible-object-input-layout]]`). Поэтому путь обязан иметь выключатель и авто-откат по образцу
`da_render_tree_batches` («нет нужных констант в шейдере — рисуем по-старому»), а не «должно
работать».
