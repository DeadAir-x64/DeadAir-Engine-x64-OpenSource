...
- **гварды реестра ALife** — единственный пункт, где нужен конкретный сценарий, а не просто игра:
  переходы между локациями (гвард двойной регистрации) и загрузка старого сейва (гвард выхода за
  границы). Обе защиты молчаливы; проверка = отсутствие строк `[DA_PORT] ALife:` в логе. Загрузка
  квиксейва на 27 394 объекта 29 июля прошла чисто, смена уровня ещё не гонялась.
...
дрожь растительности, плетение детальной текстуры и мерцание земли.
...
**Известных незакрытых визуальных дефектов не осталось.** Три последних закрыты и подтверждены в
игре: дрожь растительности, плетение детальной текстуры под FSR 2 и мерцание земли под острым углом.
Последние два лечит одна и та же настройка — `r__tf_aniso 16`; механизм и следствия для пресетов
разобраны в `09_UPSCALERS.md`, история замеров осталась в `10_DEBUG_DETAIL_WEAVE.md`.
