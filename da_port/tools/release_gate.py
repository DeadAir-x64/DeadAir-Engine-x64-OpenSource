...
def check_trees():
    ...
    for name, repo in (('xray-16', XRAY), ('обновление', UPDATE)):
        ...
        if ptrs:
            ...
            # LuaJIT туда не попадают. Лечится только своими форками, куда эти ветки можно
            # выложить; до тех пор — знать и не забывать.
            for p in ptrs:
                print('         %s: указатель на upstream, локально своя ветка' % p)
            ok = say('подмодули %s' % name, False,
                     'указателей расходится: %d — публичные исходники не воспроизводят сборку'
                     % len(ptrs)) and ok
    return ok
