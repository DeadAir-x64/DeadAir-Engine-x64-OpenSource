...
// [DA_PORT] Пресет модели DLSS. Ноль - не задавать (поведение по умолчанию), иначе значение
// NVSDK_NGX_DLSS_Hint_Render_Preset: 10 = J, 11 = K, 12 = L, 13 = M.
//
// Зачем ручка. Пресет выбирает МОДЕЛЬ реконструкции, а не её настройки, и разница между моделями
// как раз в устойчивости тонких высокочастотных деталей. Заголовок SDK прямо пишет про J: "может
// давать чуть меньше шлейфов ЦЕНОЙ ДОПОЛНИТЕЛЬНОГО МЕРЦАНИЯ, K обычно предпочтительнее". До этой
// правки мы не задавали пресет вовсе, то есть брали Default - а он, по тому же заголовку, "может
// меняться после обновления драйвера". Иначе говоря, модель нам выбирал драйвер, и молча.
//
// Действует только на СЛЕДУЮЩЕЕ создание фичи: NGX читает пресет один раз, при создании.
DA_NGX_API void da_ngx_set_preset(int preset);
