...
void INetQueue::Release()
{
    ...
    //
    // Верхнюю границу пул держит сам: он не растёт выше пикового числа одновременных сообщений.
    // Прежнее прореживание «удалять лишние, если минуту ничего не создавалось» не восстанавливаю —
    // в живой игре пакеты идут каждый кадр, условие не выполняется никогда, и смысла в нём нет.
