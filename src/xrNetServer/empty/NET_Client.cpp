...
void INetQueue::Release()
{
    ...
    // [DA_PORT] ПОЧИНЕНА УТЕЧКА. Здесь возврат пакета в пул был закомментирован, а сам указатель
    // выбрасывался через pop_front — то есть КАЖДЫЙ обработанный пакет терялся навсегда. А это
    // NET_Packet целиком: 16405 байт, из которых используются два десятка.
    //
    // Как нашли: обход куч показал рост живых аллокаций на 45 МБ за перезагрузку сохранения,
    // гистограмма по размерам дала подпись — блоки ровно по 16413 байт (16405 плюс заголовок
    // аллокатора), внутри M_EVENT с GE_WPN_STATE_CHANGE, ловушка в аллокаторе назвала
    // IPureClient::OnMessage. Счётчики пула при этом честно показывали нули: пул пуст всегда,
    // потому что в него ничего не возвращается, и Create() каждый раз выделяет заново.
    //
    // ⚠️ [DA_PORT] Уточнение 18.08: «пул не растёт выше пика» — это и есть храповик.
    //
    // Пик берётся за ВСЮ сессию: один всплеск (массовый спавн при входе на уровень, выброс) навсегда
    // поднимает полку, и вернуть её нечем. Ячейка — NET_Packet с буфером 16 КБ, так что три тысячи
    // одновременных сообщений это около полусотни мегабайт до конца игры. Прежнее прореживание по
    // времени я не восстанавливаю (оно и правда мёртвое), но потолок ставлю: запас в 256 ячеек
    // (~4 МБ) кадру хватает с избытком, всё сверх него отдаём сразу.
