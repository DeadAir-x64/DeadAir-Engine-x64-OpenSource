...
//
// ⚠️ Эта проверка закрывает однопоточную часть зазора и случай нулевой формы. Код здесь стоковый,
// у OpenXRay та же дыра.
//
// ⛔ ПОПРАВКА ПО ЗАМЕРУ. Выше в этом комментарии стояло «рабочий поток успел прочитать указатель»,
// а ниже — «сюда приходят несколько рабочих потоков». И то и другое оказалось НЕВЕРНО: прибор
// da_cform_note_thread (xr_collide_form.cpp) за 1800 кадров живого уровня насчитал в проверке луча
// по скелету РОВНО ОДИН поток. Многопоточна регистрация «ворон» (там честный атомарный CAS и списки
// по воркерам) — это другая подсистема, и её спутали с запросами.
//
// Значит объяснение вылета «указатель освобождён другим потоком» не подтверждается, а настоящей
// причиной остаётся порядок в net_Destroy — он с тех пор исправлен.
//
// ⛔ ВТОРАЯ ПОПРАВКА, тоже по замеру, и она про ЦЕНУ МОЛЧАНИЯ. Счётчик ниже за прогон не щёлкнул
// ни разу, и из этого напрашивался вывод «окно редкое». Вывод НЕОБОСНОВАН: прямой счётчик вызовов
// показал, что за 1800 кадров живого уровня da_query_cform вызывается ОДИН раз. То есть молчал не
// зазор, а весь путь — динамическая ветка запроса луча в этом сценарии просто не работает.
//
// ⚠️ Отсюда требование к любой будущей проверке ЭТОГО кода: обычная загрузка сейва его не задевает.
// Нужен сценарий, где ИИ и правда строит линии видимости сквозь динамические объекты, то есть бой.
// Основную долю проверок луча по скелетам (около одной на кадр) даёт вовсе не CObjectSpace, а
// Feel_Vision.cpp:244 — он зовёт GetCForm()->_RayQuery НАПРЯМУЮ, минуя этот помощник; на ноль форму
// он при этом проверяет, а xr_delete обнуляет указатель, так что дыры там нет.
IC ICollisionForm* da_query_cform(IGameObject* collidable)
{
    ...
    if (collidable->getDestroy() || !collidable->GetCForm())
    {
        ...
        //
        // Счётчик оставлен атомарным, хотя замер показал один поток: цена нулевая, а день, когда
        // запросы всё-таки разнесут по потокам, счётчик переживёт молча. Имя объекта НЕ печатается
    ...
    do
    {
        ...
        if (R.tgt & d_mask)
        {
            ...
            for (auto& p_spatial : r_spatial)
            {
                ...
                if (collidable == ignore_object)
                    continue;
                // [DA_PORT] Помощник поставлен для единообразия: здесь форма бралась напрямую, а на
                // четырёх других обходах защита уже стояла.
                //
                // ⛔ Честно: цена правки почти нулевая, потому что _RayQuery3 — МЁРТВЫЙ КОД.
                // Вызовов у него нет ни одного (диспетчер CObjectSpace::RayQuery уходит в
                // _RayQuery2), остались только объявление в xr_area.h и это определение. Оставлено
                // не ради защиты, а чтобы функция не оказалась единственной незащищённой в тот день,
                // когда её кто-нибудь оживит.
                ICollisionForm* cform = da_query_cform(collidable);
                if (!cform)
                    continue;
                ECollisionFormType tp = cform->Type();
