...
// [DA_PORT] Сброс блокировки ввода при старте уровня — иначе она переживает смерть и загрузку.
//
// `level.disable_input()` взводит ГЛОБАЛЬНЫЙ флаг `g_bDisableAllInput` (Level_input.cpp), и снять его
// может только парный `level.enable_input()` из скрипта. Мод гасит ввод на время анимаций приёма
// предметов — в actor_effects_data.script это пары вида `[0] = disable_input()` … `[5570] =
// enable_input()`, то есть возврат висит на ТАЙМЕРЕ.
//
// Если сессия оборвалась между этими двумя точками — актёр умер, игрок загрузил сохранение, сменился
// уровень, — таймер не доживает до срабатывания, а флаг остаётся взведённым. Движок его нигде не
// сбрасывал, поэтому после загрузки игрок оказывался в живом, работающем мире, но не мог ни идти, ни
// стрелять. Escape при этом работал (Level_input.cpp:137 пропускает его отдельно), что и делало
// картину непонятной: меню открывается, значит «игра не висит», а управления нет.
//
// Ровно так это и воспроизвелось у Павла: смерть после промотки времени → «загрузить последнее
// сохранение» → двинуться нельзя.
//
// Старт уровня — правильное место: он выполняется и при новой игре, и при загрузке сохранения, и при
// переходе между локациями, то есть закрывает все три способа потерять парный вызов. Скрипт, которому
// блокировка нужна по делу, взведёт её заново уже после старта.
