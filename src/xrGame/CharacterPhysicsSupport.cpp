...
void CCharacterPhysicsSupport::in_UpdateCL()
{
    ...
    else if (ik_controller())
    {
        ...
        // [DA_PORT] Инверсная кинематика ног — только вблизи. Ручка ph_ik_dist, в метрах.
        //
        // Замером (da_stalker_dump) физика оказалась самой дорогой фазой обновления сталкера: 1.05 мс
        // на кадр при 66-82 сталкерах, то есть больше половины их стоимости и шестая часть всего
        // кадра. У живого NPC вся эта физика сводится вот к этому вызову: подгонка стоп к рельефу —
        // лучи в геометрию уровня на каждую ногу, каждый кадр, независимо от того, видно его или нет.
        //
        // Почему пропуск безопасен. Тяжёлый пересчёт костей (Calculate) висит на ВИЗУАЛЬНОМ колбэке
        // и потому и так привязан к отрисовке; здесь пропускается только сбор данных о земле под
        // ногами. Стопы у дальнего NPC остаются на последних вычисленных отметках — на расстоянии,
        // где нога занимает считанные пиксели, это не видно. Подойдёт ближе порога — подгонка
        // возобновится со следующего же кадра.
        //
        // Порог, а не прореживание по кадрам: у прореживания стопы дёргались бы рывками при
        // движении NPC, а тут они просто перестают уточняться.
        //
        // ph_ik_dist 0 возвращает прежнее поведение точь-в-точь — на случай, если где-то это всё же
        // окажется заметно.
        extern float ps_da_ik_dist;
        const bool ik_near = ps_da_ik_dist <= 0.f ||
            Device.vCameraPosition.distance_to_sqr(m_EntityAlife.Position()) <=
                ps_da_ik_dist * ps_da_ik_dist;
        if (ik_near)
