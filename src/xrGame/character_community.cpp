...
// [DA_PORT] Отношения группировок переживали новую игру, и это было заметно в игре.
//
// ЧТО БЫЛО. m_relation_table — статическая таблица: строится один раз при первом обращении из
// [communities_relations] и живёт до выгрузки модуля, то есть до выхода из игры. Скрипты правят её
// на ходу: game_relations.script при убийствах опускает отношение группировки к актёру, вплоть до
// -3000 без пути назад. Новая игра таблицу не трогала — ни здесь, ни где-либо ещё. Отсюда жалоба:
// перестрелял Долг в одном прохождении, начал новое не выходя из игры — Долг враждебен с порога.
//
// ЧТО СТАЛО. Таблица освобождается в начале КАЖДОЙ игры (CALifeSimulator::CALifeSimulator), новой и
// загружаемой; следующее обращение соберёт её заново из конфига. Сейв при этом ничего не теряет:
// своё он хранит отдельно и накатывает поверх сам (game_relations.load_actor_relations на on_game_load).
//
// Почему сброс безусловный, а не только для новой игры: иначе загрузка старого прохождения оставляла бы
// таблицу заражённой, и следующая новая игра в той же сессии опять начиналась бы с чужой враждой.
//
// Соседние таблицы (ранги, репутация, монстры) трогать незачем — у них нет ни одного места записи
// во время игры, они только читаются.
void CHARACTER_COMMUNITY::da_reset_relations() { m_relation_table.clear(); }
