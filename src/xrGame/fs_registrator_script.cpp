...
public:
    ...
    // [DA_PORT] Перечень может быть ПУСТЫМ УКАЗАТЕЛЕМ, и проверить это из скрипта нельзя.
    //
    // file_list_open возвращает nullptr, когда папки нет. Обёртка при этом остаётся полноценным
    // объектом, и в Lua он ИСТИННЫЙ — привычная защита `if list and list:Size()` не срабатывает,
    // потому что проверяется обёртка, а не то, что внутри. Дальше Size() разыменовывает ноль, и
    // игра падает с чтением по адресу 8.
    //
    // Поймано на паркуре: он перебирает папку со звуками, которой у нас нет, и защита в самом моде
    // стояла — но помочь не могла. Мина общая: так падает ЛЮБОЙ скрипт, заглянувший в
    // отсутствующую папку, а имя папки приходит из данных мода и меняется без правки кода.
    //
    // Пустой перечень — законный ответ «ничего не нашлось», а не ошибка вызывающего.
    u32 Size() { return m_p ? u32(m_p->size()) : 0u; }

    // Заодно и границы: at() на пустом перечне бросил бы исключение, которое здесь никто не ловит.
    LPCSTR GetAt(u32 idx) { return (m_p && idx < m_p->size()) ? (*m_p)[idx] : nullptr; }
