...
// [DA_PORT] Номер кости из конфигурации, приведённый к годному.
//
// ⛔ Здесь была не «возможная ошибка», а ЗАПИСЬ ЗА ГРАНИЦЫ МАССИВА. Имя кости берётся из секции
// NPC (`bone_head` / `bone_shoulder` / `bone_spin`), то есть из данных мода. Если в модели такой
// кости нет — а несовпадение модели и конфига в наборах сталкеров дело обычное, — `LL_BoneID`
// возвращает `BI_NONE` (0xFFFF). Дальше `LL_GetBoneInstance(0xFFFF)` берёт `bone_instances[65535]`
// и в него ЗАПИСЫВАЕТСЯ указатель на обработчик: это примерно восемь мегабайт за концом массива,
// то есть порча чужой кучи. Падало бы потом и в другом месте, а стек называл бы жертву.
//
// Единственная проверка, `VERIFY(bone_id < LL_BoneCount())` внутри `LL_GetBoneInstance`, в релизе
// пуста. Тот же класс, что задача #33.
static CBoneInstance* da_config_bone(IKinematics* kinematics, pcstr section, pcstr key)
{
    ...
    if (bone_id == BI_NONE || bone_id >= kinematics->LL_BoneCount())
    {
        ...
        if (da_hits <= 10 || (da_hits % 500) == 0)
            Msg("! [DA] %s: кости «%s» (%s) в модели нет, поворот не назначен (случаев %u)", section, bone_name,
                key, da_hits);
        return nullptr;
    }
...
void CStalkerAnimationManager::assign_bone_callbacks()
{
    ...
    if (CBoneInstance* bone = da_config_bone(kinematics, section, "bone_head"))
    ...
    if (CBoneInstance* bone = da_config_bone(kinematics, section, "bone_shoulder"))
    ...
    if (CBoneInstance* bone = da_config_bone(kinematics, section, "bone_spin"))
