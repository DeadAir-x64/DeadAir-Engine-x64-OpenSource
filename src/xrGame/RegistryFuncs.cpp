...
bool ReadRegistryValue(LPCSTR rKeyName, DWORD rKeyType, void* value)
{
    ...
    {
        // [DA_PORT] Отсутствие ключа — норма, а не ошибка, и печаталась она четыре раза за запуск.
        //
        // Читается тут профиль сетевой игры из реестра розничного «Зова Припяти» (login_manager,
        // сеть GameSpy). У самостоятельной сборки этого ключа нет и быть не должно: розничная игра
        // не установлена, а сеть всё равно отключена. Тестер же видит строку с восклицательным
        // знаком и сообщает о ней как о проблеме — то есть цена сообщения выше его пользы.
        //
        // Оставляем один раз и пояснением, а не четыре раза как об ошибке.
        static bool reported = false;
        if (!reported)
        {
            reported = true;
            Msg("~ [DA_PORT] розничный STALKER не установлен (%s) - профиль сетевой игры не читается, "
                "для одиночной это нормально",
                REGISTRY_PATH);
        }
        return false;
    }
