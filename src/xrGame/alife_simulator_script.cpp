...
void CALifeSimulator__release(CALifeSimulator* self, CSE_Abstract* object, bool)
{
    ...
    // [DA_PORT] Указатель может быть ВИСЯЧИМ, а не нулевым — и это разные вещи.
    //
    // Проверка выше ловит только ноль. Скрипт же держит `CSE_Abstract*`, полученный раньше, и между
    // получением и вызовом объект мог быть снят: в ремонте (ui_itm_repair.script) за один заход и
    // создают предмет, и освобождают другой, а рекурсивное освобождение тянет за собой детей.
    //
    // ⚠️ Дальше по функции стоит `smart_cast`, то есть `dynamic_cast`, и он читает таблицу методов.
    // По освобождённой памяти это обращение по мусору. Хуже того, оно стоит ПЕРЕД
    // CALifeSimulatorBase::release, который от повторного захода уже защищён — то есть весь тот
    // заслон обёртка обходит.
    //
    // Метка жизни (CSE_Abstract::da_object_alive) читает обычное поле, без единого виртуального
    // вызова, поэтому спросить ею можно и по мёртвому указателю. Разбор — у самой метки.
    if (!object->da_object_alive())
    {
        Msg("~ [DA_PORT] ALife: скрипт просит освободить УЖЕ СНЯТЫЙ объект — пропущено");
        return;
    }
