...
//#endif
    ...
    // [DA_PORT] Пересчёт веса ОБЯЗАТЕЛЕН и здесь.
    //
    // Он стоял в Clear, Take, DropItem, Belt и Ruck, но не в Slot — и это было верно, пока слот и
    // рюкзак весили одинаково. Наша же правка (CalcTotalWeight, EQUIPPED_WEIGHT_FACTOR) развела их:
    // надетое считается за 30%, лежащее в рюкзаке за 100%. С этого момента перенос В СЛОТ меняет
    // общий вес, а пересчёта не было.
    //
    // Как проявлялось: паркур убирает оружие в рюкзак (вес растёт, Ruck пересчитывает) и по
    // окончании возвращает в слот — а вот тут пересчёта нет, и завышенный вес висит дальше. Игрок
    // получал перегруз и не мог идти, пока не откроет рюкзак: любое действие в нём проходит через
    // Take или Ruck и пересчёт наконец случается.
    CalcTotalWeight();
...
bool CInventory::CanTakeItem(CInventoryItem* inventory_item) const
{
    ...
    //
    // ⚠️ ТОЛЬКО ДЛЯ ИГРОКА, и это принципиально. Отказ отсюда не «не подбирается» — он уходит
    // обратно как GE_OWNERSHIP_REJECT, и предмет ПАДАЕТ ПОД НОГИ (ai_stalker_events.cpp, ветка
    // GE_OWNERSHIP_TAKE). Пока запрет был общим, он сам работал вторым источником разбросанных по
    // локации фонарей: `sr_light.script` создаёт сталкеру фонарь, когда `stalker:object` его не
    // видит, а увидеть он может не сразу — клиентский объект появляется позже серверного. Второй
    // фонарь отвергался и оставался лежать.
    //
    // Сталкеру два фонаря безвредны: его лампы горят по одному признаку torch1, а скрипт настраивает
    // и включает только первый найденный. Ломался от дубля именно игрок — у него свет собирается из
    // настроек, приходящих в конкретный объект.
    if (smart_cast<CActor*>(m_pOwner) && smart_cast<CTorch*>(inventory_item))
    {
