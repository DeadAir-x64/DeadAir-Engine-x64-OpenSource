...
static void DA_AppendWeaponConditionType(xr_string& text, CInventoryItem& item)
{
    ...
    // [DA_PORT] Что ИМЕННО читает подсказка: номер объекта и маска.
    //
    // Жалоба: снять обвес с трупного ствола -- поломки исчезают. Запись маски в серверный объект
    // подтверждена логом (23 назначения, ноль отказов), обнулений внутри объекта прибор не видел.
    // Значит остаются два объяснения, и различает их только НОМЕР: тот же объект с обнулённой
    // маской -- значит её кто-то стёр; другой номер -- значит оружие пересоздали, и подсказка
    // показывает уже не тот ствол.
    //
    // Печатаем только при СМЕНЕ пары: подсказка рисуется каждый кадр наведения.
    {
        static u16 last_id = 0xffff;
        static u32 last_mask = u32(-1);
        if (w->ID() != last_id || mask != last_mask)
        {
            last_id = w->ID();
            last_mask = mask;
            Msg("~ [DA_WPN] подсказка: %s id[%u] маска %u", w->cNameSect().c_str(), u32(w->ID()), mask);
        }
    }
