...
bool CUIInventoryCellItem::EqualTo(CUICellItem* itm)
{
    ...
    // [DA_PORT] Склейка одинаковых предметов с РАЗНЫМ состоянием — по ручке da_stack_any_condition.
    //
    // Почему это вообще вопрос. В ящике одинаковые бинты и консервы лежат одной ячейкой с «x14», а в
    // рюкзаке теми же предметами забита половина сетки поштучно. Флаг склейки тут ни при чём: он
    // стоит и у dragdrop_bag, и у dragdrop_deadbody_bag (actor_menu_16.xml). Разделяет их вот эта
    // строка: у еды и медикаментов в Dead Air есть порча, и состояние тикает у каждого предмета
    // своим ходом от момента подбора. Два бинта, взятые с разницей в минуту, отличаются больше чем
    // на сотую — и в одну ячейку уже не сходятся. У патронов состояния нет, поэтому они
    // склеиваются и сейчас: на скриншоте инвентаря числа стоят ровно на них.
    //
