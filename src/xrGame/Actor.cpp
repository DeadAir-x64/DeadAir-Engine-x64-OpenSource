...
// [DA_PORT] Ограничитель разворота корпуса относительно взгляда, в градусах. 0 — выключен.
//
// Симптом: при быстром повороте в кадр влезает собственное плечо. Причина не в сдвиге, а в том, что
// ориентация перволичной модели берётся от АКТЁРА, а тот догоняет камеру с инерцией: пока он
// доворачивается, корпус остаётся развёрнутым поперёк взгляда и попадает в поле зрения.
//
// В отрасли это решают «рулевой» связкой камера↔тело с ЗАЖИМОМ угла между ними: тело доворачивается
// не мгновенно, но и разойтись со взглядом больше чем на предел не может. У Froyok в разборе
// перволичной камеры для UE4 зажим ±91°, но там торс докручивается аддитивной позой (aim offset) и
// сама модель нарисована так, чтобы плечо в кадр не лезло. У нас докрутить позвоночник нечем —
// поза снимается с актёра как есть, — поэтому доворачиваем модель целиком и предел берём заметно
// меньше: 40° это ещё естественное «тело отстаёт», но уже не «вижу своё плечо».
//
// Сам актёр не трогается: правка чисто отображательная, на стрельбу и физику не влияет.
float g_da_legs_yaw_limit = 40.0f;
...
void CActor::renderable_RenderLegs(u32 context_id, IRenderable* root)
{
    ...
    // [DA_PORT] Зажим угла между корпусом и взглядом — разбор у объявления g_da_legs_yaw_limit.
    if (g_da_legs_yaw_limit > 0.f)
    {
        Fvector cam_f = Device.vCameraDirection;
        cam_f.y = 0.f;
        Fvector mdl_f = xf.k;
        mdl_f.y = 0.f;
        if (cam_f.magnitude() > EPS_S && mdl_f.magnitude() > EPS_S)
        {
            cam_f.normalize();
            mdl_f.normalize();
            // Знаковый угол «на сколько модель отстала от взгляда» вокруг вертикали.
            float d = atan2f(cam_f.x, cam_f.z) - atan2f(mdl_f.x, mdl_f.z);
            while (d > PI)
                d -= PI_MUL_2;
            while (d < -PI)
                d += PI_MUL_2;

            const float lim = deg2rad(g_da_legs_yaw_limit);
            if (_abs(d) > lim)
            {
                // Доворачиваем ровно на превышение: у предела модель «прилипает» к взгляду и едет
                // за ним, внутри предела отставание остаётся как было.
                Fmatrix rot;
                rot.rotateY(d > 0.f ? d - lim : d + lim);
                const Fvector keep = xf.c;
                Fmatrix turned;
                turned.mul_43(rot, xf);
                turned.c = keep; // поворот вокруг СВОЕГО начала, положение не трогаем
                xf = turned;
            }
        }
    }
