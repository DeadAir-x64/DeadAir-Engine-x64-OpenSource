...
void CAgentEnemyManager::distribute_enemies()
{
    ...
    // [DA_PORT] Раз на отряд за кадр, а не на каждого бойца.
    //
    // Раздача считает ОДНО И ТО ЖЕ на весь отряд: собрать врагов у всех членов, оценить опасность,
    // распределить цели, расставить маски. А зовёт её каждый боец при каждом обновлении своей
    // памяти -- то есть отряд пересчитывает распределение столько раз за кадр, сколько в нём
    // бойцов. Результат при этом один и тот же: мир между двумя вызовами в одном кадре не менялся.
    //
    // Найдено замером da_memory_dump после боя: работа с врагами занимала 93.9% всей памяти NPC,
    // и 95% этой работы приходилось ровно на раздачу -- 3.58 мс на кадр при 850 вызовах на 300
    // кадров. Выбор цели и повторный разбор рядом с ней стоят нуля.
    //
    // Цена: боец, чьё обновление пришлось на этот же кадр позже, получит распределение, снятое
    // мгновением раньше. Это ровно та точность, с которой оно и так работает -- члены отряда
    // обновляются вразнобой и всегда видят чуть разное состояние.
    if (m_da_distribute_frame == Device.dwFrame)
        return;
    m_da_distribute_frame = Device.dwFrame;
