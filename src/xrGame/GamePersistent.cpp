...
void CGamePersistent::OnFrame()
{
    ...
    //
    // С открытым главным меню уровень не тикает вовсе, и первые два прогона воспроизведения молча
    // простояли в меню: команда была взведена, а счётчик кадров не двигался. Этот же кадровый
    // обработчик идёт всегда, поэтому отсюда можно и меню закрыть, и сценарий продолжить.
    DA_AfterLoadTick();

    // [DA_PORT] Окно замера счётчика выделений — здесь же и по той же причине.
    DA_AllocStatTick();

    // [DA_PORT] И стартовая команда из `-da_cmd` — тем же обработчиком и по той же причине: он
    // идёт и в меню, и в игре, а значит сценарий можно начать, не дожидаясь уровня.
    DA_StartupCommandTick();
...
void CGamePersistent::OnEvent(EVENT E, u64 P1, u64 P2)
{
    ...
    if (E == eQuickLoad)
    {
        ...
        // [DA_PORT] Погасить все звучащие эмиттеры перед сносом мира.
        //
        // Движок делает это в CGamePersistent::Disconnect, то есть при выходе с уровня. Но
        // перезагрузка сохранения ИЗНУТРИ игры туда не заходит: уровень остаётся, объекты сносятся,
        // симулятор пересоздаётся — а звуки прошлого мира продолжают играть. Замер поймал это
        // прямо: ambient/fire2_extinguished и detectors/da-2_beep1 прибавляли РОВНО по пять копий
        // на каждую перезагрузку, восемь раз подряд (10 15 20 25 30 35 40 45), тогда как остальные
        // звуки стояли на месте, а объекты, ALife, частицы, текстуры и строки не менялись вовсе.
        GEnv.Sound->stop_emitters();
