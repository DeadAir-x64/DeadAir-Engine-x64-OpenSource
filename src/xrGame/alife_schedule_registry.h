...
// [DA_PORT] Числится ли объект с этим номером в реестре ALife. Определение в
// alife_schedule_registry.cpp: тянуть alife_simulator.h сюда нельзя — будет круг включений.
...
private:
    struct CUpdatePredicate
    {
        ...
        {
            // [DA_PORT] Обновляем только то, что реестр ALife ещё держит за этим номером.
            //
            // Расписание — не единственный список, где объект числится, и снимают его оттуда не
            // все пути освобождения. Скриптовый release для ОНЛАЙН-объекта, например, вообще не
            // зовёт release: он шлёт сетевой пакет GE_DESTROY и уходит, а дальше объекта не стало,
            // но запись в расписании осталась. В логе это видно по «SV:ge_destroy: [id] not found
            // on server» и по повторным «releasing object» с одними и теми же номерами.
            //
            // Итог — планировщик зовёт update() у трупа, тот уходит в Lua, скрипт зовёт обратно
            // commander_id(), и падение приходит оттуда:
            //   CSE_ALifeOnlineOfflineGroup::commander_id(), чтение по ffffffffffffffff
            //
            // Проверка стоит именно здесь, в точке использования, а не в очередном месте снятия:
            // мест снятия много и каждое со своим условием, а точка использования одна. Стоит она
            // один поиск в map на объект за цикл — против виртуального вызова по мёртвой памяти.
            // [DA_PORT] Сначала — метка жизни самого объекта, и только потом всё остальное.
            //
            // Сверка по реестру (ниже) закрывает случай «номер переиспользован», но бессильна,
            // когда реестр держит тот же висячий указатель: сравниваются два одинаковых мусора
            // и совпадают. Метка гасится в деструкторе CSE_ALifeSchedulable, поэтому не зависит
            // ни от реестра, ни от того, каким путём объект удалили.
            if (!(*i).second || !(*i).second->da_schedulable_alive())
            {
                static u32 s_da_dead = 0;
                ++s_da_dead;
                if (s_da_dead <= 5 || (s_da_dead % 100) == 0)
                    Msg("~ [DA_SCHED] в расписании мёртвая запись: номер [%d], пропущено, всего %u",
                        (*i).first, s_da_dead);
                return;
            }

            if (!da_alife_object_registered((*i).first, (*i).second))
                return;
