...
//	Msg					("* CLIENT: Spawn: %s, ID=%d", *E->s_name, E->ID);
    ...
    // [DA_PORT] Кто создаётся прямо сейчас — по ручке da_spawn_trace.
    //
    // Падение внутри спавна по стеку не разобрать: быстрая сборка идёт без карты линковщика, и
    // символизатор подставляет ближайший символ, то есть заведомо чужое имя. Секция объекта отвечает
    // на тот же вопрос без догадок. Печать ДО создания: если упадёт в нём, последняя строка в логе и
    // назовёт виновника.
    extern int ps_da_spawn_trace;
    if (ps_da_spawn_trace)
        Msg("~ [DA_SPAWN] создаю [%s] id [%u] родитель [%u]", E->s_name.c_str(), u32(E->ID),
            u32(E->ID_Parent));
    ...
    LPCSTR section, const Fvector& position, u32 level_vertex_id, u16 parent_id, bool return_item)
{
    ...
    if (dynamic_object && ai().get_level_graph())
    {
        ...
        {
            // [DA_PORT] Негодная вершина: предмет уходит на сервер без графовой привязки.
            //
            // m_tGraphID остаётся тем, чем его оставил конструктор, а по нему потом идёт регистрация
            // в графовом реестре — по ИНДЕКСУ вектора (см. alife_graph_registry). Чем это кончается,
            // мы уже знаем: в лучшем случае предмет теряется, в худшем правится чужая память.
            //
            // Ловим это здесь, а не по последствиям: тут ещё известны и секция, и владелец, а через
            // три слоя не останется ни того, ни другого. Подозрение возникло на жалобе «при первом
            // открытии у торговца пусто»: товар спавнится именно так — Level().spawn_item с вершиной
            // владельца, а у только что вышедшего в онлайн NPC она может быть ещё не вычислена.
            static u32 s_da_bad_vertex = 0;
            ++s_da_bad_vertex;
            if (s_da_bad_vertex <= 10 || (s_da_bad_vertex % 50) == 0)
                Msg("! [DA_PORT] спавн [%s] с негодной вершиной [%u] (владелец id %d), случай #%u",
                    section, level_vertex_id, parent_id, s_da_bad_vertex);
        }
    }
