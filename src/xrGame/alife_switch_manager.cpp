...
// [DA_PORT] Удаление отложено до конца обхода — иначе оно рушит обход, из которого вызвано.
//
// release() РЕКУРСИВНО убирает объект вместе с детьми (труп уносит с собой весь свой инвентарь) и
// вычёркивает их из реестра уровня. А зовут нас из обхода этого самого реестра, объект за объектом.
// Ребёнок, удалённый вместе с родителем, остаётся дальше по списку обхода — и на следующем шаге
// switch_object получает указатель на освобождённую память. Первое же обращение здесь виртуальное
// (redundant()), а виртуальный вызов по освобождённому объекту — это переход по НУЛЕВОМУ адресу:
// аллокатор обнуляет указатель на таблицу методов. В логе это выглядит как «исполнение по адресу
// 0000000000000000» с пустым стеком — разматывать нечего, кадр уже разрушен.
//
// Мина лежала невзведённой всё время существования мода: интервалы уборки тел стояли в 65535 часов,
// то есть redundant() не возвращал true никогда и release() из этого обхода не вызывался. Включили
// уборку по таймеру — и она взвелась.
//
// Копим идентификаторы, а не указатели: пока очередь ждёт, объект может быть удалён по другой
// причине, и указатель протухнет. По идентификатору же поиск в реестре просто не найдёт объект, и
// это безопасно.
