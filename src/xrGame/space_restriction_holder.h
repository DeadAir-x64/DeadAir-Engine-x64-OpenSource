...
// [DA_PORT] ОДИН замок на всю подсистему пространственных ограничителей.
//
// Каждый её слой кэширует ЛЕНИВО, при первом обращении: у ограничителя — списки сфер и коробок,
// у ограничения — граница, у держателя и менеджера — карты клиентов. И всё это общее.
//
// Пока весь параллельный кадр шёл одной задачей, это сходило с рук. Мы задачи сгруппировали по
// ключу совместности — и теперь несколько построителей пути добираются до ХОЛОДНОГО ограничителя
// одновременно, а перестроение идёт на месте: векторы перевыделяются под соседом. Симптом —
// запись в чужую кучу через кадр после сброса уровня, то есть далеко от причины.
//
// Замок РЕКУРСИВНЫЙ намеренно: слои зовут друг друга (менеджер -> ограничение -> ограничитель),
// и захват берётся на каждом входе в общее состояние. Ленивое кэширование при этом остаётся —
// сериализуется только доступ к нему.
//
// Найдено не у себя: Dead Air Refined, коммит 3f7aa0bf от 10.08.2026 (вылет на сейве игрока с
// востока Припяти).
