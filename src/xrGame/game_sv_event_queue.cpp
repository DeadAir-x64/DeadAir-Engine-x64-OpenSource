...
void GameEventQueue::Release()
{
    ...
    //---------------------------------------------
    // [DA_PORT] Прореживание было мёртвым, а пул — храповиком.
    //
    // Условие требовало, чтобы МИНУТУ никто не создавал ни одного события: в живой игре они идут
    // потоком, и оно не выполнялось никогда. Пул поэтому рос до всевременного максимума и назад не
    // возвращался: один всплеск (массовый спавн при входе на уровень, выброс) навсегда поднимал
    // полку. Ячейка несёт NET_Packet с буфером 16 КБ, то есть три тысячи одновременных событий —
    // около полусотни мегабайт, удерживаемых до конца сессии.
    //
    // Держим запас в 256 ячеек (~4 МБ) — покрывает обычный кадр с большим запасом, а всё сверх него
    // отдаём сразу. Условие по времени оставлено вторым поводом: если пауза всё же случилась,
    // прорежаем и малый пул.
    u32 tmp_time = CPU::GetTicks() - 60000;
    u32 size = unused.size();
    if (size > 256 || ((LastTimeCreate < tmp_time) && (size > 32)))
    {
