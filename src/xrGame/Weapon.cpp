...
//
// Источников три, и они разной природы — потому и собраны под одну ручку:
//   • zoom_dof из секции ствола — эффектор на вскидке (OnZoomIn);
//   • reload_dof / reload_empty_dof — эффектор на перезарядке (OnStateSwitch);
//   • наводка по лучу (SetPickableEffectorDOF) — перестройка глубины резкости под то, во что
//     смотрит прицел, каждый кадр. Вот это и даёт основное «плывёт мир при вскидке», причём
//     независимо от того, есть ли у ствола zoom_dof в конфиге вообще.
//
// Правка не трогает конфиги — она гасит применение, поэтому значения в секциях оружия остаются на
// месте и включаются одной командой обратно.
//
...
void CWeapon::OnZoomIn()
{
    ...
    // [DA_PORT] Это и есть ГЛАВНОЕ размытие при прицеливании, а вовсе не zoom_dof выше.
    //
    // Режим «наводки по лучу»: пока игрок целится, глубина резкости каждый кадр перестраивается под
    // расстояние до того, во что смотрит прицел (CGamePersistent::UpdateDof, диапазон из секции
    // zone_pick_dof - плюс-минус 70 метров). Отсюда и ощущение, что мир плывёт при вскидке, даже
    // когда у ствола никакого zoom_dof в конфиге нет.
    //
    // Раньше оно включалось безусловно, поэтому g_weapon_dof 0 гасил только два эффектора из трёх, и
    // размытие оставалось. Теперь под той же ручкой все три: прицеливание, наводка по лучу и
    // перезарядка.
    //
    // Выключение (ниже, в OnZoomOut) намеренно осталось безусловным: если игрок сменит настройку
    // прямо в прицеле, режим обязан сняться, иначе останется висеть до следующей вскидки.
    if (g_weapon_dof && GetHUDmode())
