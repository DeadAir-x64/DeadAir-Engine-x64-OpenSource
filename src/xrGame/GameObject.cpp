...
Fvector CGameObject::get_last_local_point_on_mesh(Fvector const& local_point, u16 const bone_id) const
{
    ...
    // [DA_PORT] Объект без формы столкновений. Так бывает у уже уничтоженного объекта: net_Destroy
    // удаляет CForm (см. выше в этом файле), а из пространственной базы объект уходит следом — и в
    // зазоре между этими двумя шагами его успевает «увидеть» чужое зрение. Дальше Feel::Vision::o_new
    // зовёт этот метод, и разыменование нулевого CForm роняет игру.
    //
    // Что здесь НЕ надо чинить: сам движок такую ситуацию знает — рядом стоит проверка
    // «зарегистрирован как сталкивающийся, а модели столкновений нет». Только она под VERIFY, то есть
    // в релизе её нет вовсе, и вместо внятного сообщения получается чтение по адресу 0x20.
    //
    // Вместо падения отдаём точку как есть: она в локальных координатах объекта, и без габаритов
    // единственное осмысленное преобразование — перенос его же матрицей. Для зрения этого достаточно:
    // объект в этот момент всё равно доживает последние кадры.
    if (!CForm)
    {
        static u32 s_last_report = 0;
        if (Device.dwTimeGlobal - s_last_report > 5000)
        {
            s_last_report = Device.dwTimeGlobal;
            Msg("~ [DA_PORT] зрение: у объекта '%s' нет формы столкновений (уничтожается?) - точка взята без габаритов",
                cName().c_str());
        }
        M.transform_tiny(result, local_point);
        return result;
    }
