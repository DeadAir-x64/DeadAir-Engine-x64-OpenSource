...

{
    ...
    void call(IPhysicsShellHolder* obj, float min_cs, float max_cs, float& cs, float& hl, ICollisionDamageInfo* di)
    {
        ...
        // [DA_PORT] Оба указателя проверялись только `VERIFY`, то есть в релизе не проверялись.
        //
        // ⚠️ Последняя строка — это `delete this` (так и написано у авторов): снятие колбэка у
        // объекта его же и удаляет. Значит порядок важен: сведения об уроне надо тронуть ДО, а
        // после снятия к полям обращаться уже нельзя.
        VERIFY(m_object);
        if (di)
            di->SetInitiated();
        if (m_object)
...
void CPolterTele::tele_fire_objects()
{
    for (u32 i = 0; i < m_object->CTelekinesis::get_objects_total_count(); i++)
    {
        ...
        {
            // [DA_PORT] Ни актёра, ни поднятого объекта может не быть, а проверял их только
            // `VERIFY` — в релизе пусто. Актёра нет в главном меню и в зазоре при смене уровня;
            // поднятый предмет мог быть уничтожен между кадрами, пока полтергейст его «держит».
            if (!Actor())
                return;
