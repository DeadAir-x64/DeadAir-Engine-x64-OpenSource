...
#include "alife_object_registry.h" // [DA_PORT] entity_Destroy спрашивает реестр, см. ниже
#include "Level.h"
...
extern int ps_da_registry_log; // [DA_PORT] ручка da_registry_log, см. ниже
...
void xrServer::entity_Destroy(CSE_Abstract*& P)
{
    ...
    const bool destroying_object = (!ai().get_alife() || !P->m_bALifeControl);

    // [DA_PORT] ⛔ ОТВЕРГНУТО ЗАМЕРОМ: «придерживать номер за живым объектом».
    //
    // Диагноз был верен. Номер уходит в общий пул ВСЕГДА, даже когда объект остаётся жить под
    // управлением ALife, — и тогда тот же номер достаётся новому объекту. В логе это видно прямо:
    // девять подряд «реестр ALife: номер [29028] занят, запись [X] заменяется на [Y]» — один номер,
    // девять разных адресов. На ускоренном времени ловится за минуты.
    //
    // Правка (освобождать номер только вместе с объектом) убрала коллизии полностью, 9 -> 0. И тут
    // же уронила загрузку сейва: «Requesting ID has already been used!» в tfGetFromBlock. Причина в
    // том, что при загрузке объекты пересоздаются и запрашивают СВОИ прежние номера явно, через
    // tfGetID(id). Придержанный номер делает такой запрос невозможным — падение вместо коллизии.
    //
    // Чинить надо не пул, а реестр: тот, кто оставляет объект жить, обязан снять его запись. Прибор
    // ниже помечает такие случаи поимённо, чтобы искать их по следу, а не наугад.
    m_tID_Generator.vfFreeID(P->ID, Device.TimerAsync());

    if (ps_da_registry_log && !destroying_object)
    {
        static u32 s_da_id_reused = 0;
        Msg("~ [DA_REG] номер [%d] (%s) ушёл в пул, а объект остаётся жить — случай #%u",
            P->ID, P->name_replace(), ++s_da_id_reused);
    }
    ...
    // [DA_PORT] Прибор на расхождение двух реестров.
    //
    // Номер (m_tID_Generator.vfFreeID выше) возвращается в общий пул ВСЕГДА, а память освобождается
    // только когда объект уже не под управлением ALife. Значит любой путь, где release не дошёл до
    // снятия m_bALifeControl, оставляет объект ЖИТЬ с уже освобождённым номером. Дальше номер
    // выдаётся новому объекту — и получается «номер занят» с висячей записью в реестре ALife.
    //
    // Считаем такие случаи поимённо: если их ноль, версия неверна и искать надо в другом месте.
    // Прибор только считает и рассказывает — поведения он не меняет, см. отвергнутую правку ниже.
    if (ps_da_registry_log && ai().get_alife() && P->m_bALifeControl)
    {
        // Держит ли реестр ALife этот номер за этим же объектом? Если нет — мы сейчас вернём номер
        // в пул, а объект оставим жить, и номер достанется кому-то другому.
        const CSE_ALifeDynamicObject* in_alife = ai().alife().objects().object(P->ID, true);
        if (in_alife != P)
        {
            static u32 s_da_registry_divergence = 0;
            ++s_da_registry_divergence;
            Msg("~ [DA_REG] расхождение #%u: id [%d] (%s) освобождён в пул, но реестр ALife держит %s",
                s_da_registry_divergence, P->ID, P->name_replace(), in_alife ? "ДРУГОЙ объект" : "пусто");
        }
    }

    // [DA_PORT] ⛔ ОТВЕРГНУТО: «освобождать по реестру, а не по флагу».
    //
    // Замысел был верный по диагнозу: номер возвращается в пул безусловно, а память — только при
    // снятом m_bALifeControl, и когда release пропускают (объекта уже нет в реестре ALife), объект
    // остаётся жить с чужим номером. Прибор это подтвердил числом: 4669 случаев за один заход, всё
    // на мелкой живности, которую удаляют пачками.
    //
    // Правка — освобождать, если реестр ALife объект уже не держит — расхождение убрала полностью
    // (4669 -> 0). И принесла взамен ПОРЧУ КУЧИ (C0000374): часть таких объектов к этому моменту
    // уже освобождена другим путём, указатель висячий, и мы освобождали его во второй раз.
    //
    // Утечка неприятна, двойное освобождение — хуже: оно рушит кучу далеко от места ошибки и без
    // стека. Возвращено как было. Чинить надо не здесь, а там, где release пропускают вместе со
    // снятием флага, — и с проверкой, жив ли объект вообще.
    if (destroying_object)
    {
        // [DA_PORT] Снять запись из реестра ALife ПЕРЕД тем, как освободить память.
        //
        // Это не та правка, что отвергнута выше, и разница принципиальная: там мы освобождали
        // ЛИШНЕЕ и получали двойное освобождение, здесь не освобождаем ничего — только убираем
        // указатель на память, которую освободим следующей строкой.
        //
        // Зачем: реестр ALife чистится в release(), а сюда приходят и другие пути. Когда память
        // освобождена, а запись осталась, реестр отдаёт висячий указатель всякому, кто спросит по
        // номеру, — и в первую очередь скриптам, через alife_object(id). Дальше Lua зовёт метод
        // (squad:name() в sim_squad_fighting), попадает по мёртвой таблице методов и прыгает по
        // нулевому адресу: краш без стека, далеко от места настоящей ошибки.
        //
        // Условие строгое: снимаем запись, только если она указывает ИМЕННО на этот объект. Номер
        // уже возвращён в пул выше и мог достаться другому — вычеркнуть чужую живую запись значило
        // бы поменять один дефект на другой, погрубее.
        if (ai().get_alife())
        {
            if (CSE_ALifeDynamicObject* dynamic = smart_cast<CSE_ALifeDynamicObject*>(P))
            {
                // Публичен только константный доступ к реестру, а снять запись надо здесь и сейчас:
                // через секунду память станет чужой. Константность снимаем на самом реестре, объект
                // при этом не трогаем вовсе.
                const CALifeObjectRegistry& registry = ai().alife().objects();
                if (registry.object(P->ID, true) == dynamic)
                {
                    if (ps_da_registry_log)
                    {
                        static u32 s_da_stale_removed = 0;
                        ++s_da_stale_removed;
                        Msg("~ [DA_REG] снята запись #%u: id [%d] (%s) — память освобождается",
                            s_da_stale_removed, P->ID, P->name_replace());
                    }

                    const_cast<CALifeObjectRegistry&>(registry).remove(P->ID, true);
                }

                // [DA_PORT] Сторож висячих выдач: запоминаем адрес, чтобы поймать того, кто спросит
                // этот объект у реестра ПОСЛЕ освобождения. См. da_check_dangling.
                {
                    extern void da_watch_released(const void* object, u16 id);
                    da_watch_released(dynamic, P->ID);
                }
            }
        }
