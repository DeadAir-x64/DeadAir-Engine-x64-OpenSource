...
{
public:
    // [DA_PORT] Порядок ОРИГИНАЛЬНОГО X-Ray, а не OpenXRay. Тот переставил eHidden в ноль, и числа
    // 0..3 поменяли смысл — а скрипты мода сравнивают состояние с ЧИСЛОМ, не с именем.
    //
    // Мы на это уже напоролись: условие Dead Air `get_state()==0` означало «предмет в руке, покой»,
    // а стало читаться как «спрятан», из-за чего фонарь и зажигалка не загорались (обход остался в
    // itms_manager.script). Дефект молчаливый по построению — ни ошибки, ни строчки в логе, просто
    // ветка скрипта никогда не выполняется.
    //
    // Состояния 4 и выше не двигаются ни в одной из нумераций: eBore == 4 в обеих, а производные
    // состояния оружия объявлены как eLastBaseState + 1. Поэтому сравнения мода с 5/6/7 работали и
    // до этой правки — ломались только 0..3.
    enum EHudStates
    {
        eIdle = 0,
        eShowing,
    ...
    // Скрипты и данные мода завязаны на эти конкретные числа. Если кто-то снова переставит порядок,
    // сборка упадёт здесь, а не тихо в игре через месяц.
    static_assert(eIdle == 0 && eShowing == 1 && eHiding == 2 && eHidden == 3 && eBore == 4,
        "Dead Air scripts compare hud states to literal numbers");
