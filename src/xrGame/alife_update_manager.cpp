...
{
    // [DA_PORT] Скобки. Бюджет из alife.ltx задан в МИКРОсекундах (process_time = 900), а сверяется
    // он с CTimer::GetElapsed_sec() внутри CSafeMapIterator::time_over. Делитель 1000000 в исходном
    // выражении относился только к монстрячьей поправке, поэтому в реестр уходило ~899.99 — то есть
    // без малого пятнадцать минут, и проверка не срабатывала никогда.
    //
    // Так отвечает на вопрос «почему бюджет не соблюдается»: он не нарушался, его не было вовсе,
    // а строка в alife.ltx была мёртвой ручкой. Механизм разбивки работы на кадры при этом написан
    // и исправен — CSafeMapIterator продолжает обход с сохранённой позиции при следующем вызове.
    //
    // ЧЕСТНО О ПОЛЬЗЕ: на выбросы кадра правка не влияет. Восемь прогонов стенда по 3500 кадров:
    //
    //     без скобок (3 прогона)   выбросов 94, суммарно 286 мс, обновлений ALife 12460
    //     со скобками (5 прогонов) выбросов 91, суммарно 288 мс, обновлений ALife 12461
    //
    // Причина безразличия та же, что и у реестра расписания (см. alife_schedule_registry.h):
    // выбросы делает не пачка мелкой работы, а ОДИН неделимый вызов. Переключение объекта в онлайн
    // и первый за сессию спавн человека стоят по 10-12 мс каждый, а проверка времени стоит МЕЖДУ
    // объектами и разрезать такой вызов не может — она лишь раскладывает соседей по кадрам.
    //
    // Правка оставлена как исправление арифметики: пропускная способность симуляции не меняется
    // (12460 против 12461 обновления), а process_time из alife.ltx наконец начинает что-то значить.
    // Первый проход после загрузки не урезается (m_first_update), так что начальная расстановка
    // онлайн/офлайн отрабатывает целиком.
    //
    // Реестру расписания бюджет НЕ выдаётся: у него проверка времени выключена шаблоном, и это
    // решение тоже проверено замером — разбор там же.
