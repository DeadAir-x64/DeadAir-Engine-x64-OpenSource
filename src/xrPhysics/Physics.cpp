    ...
    dGeomID o1, dGeomID o2, dJointGroupID jointGroup, CPHIsland* world, const int& MAX_CONTACTS)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        // [DA_PORT] Без материала контакт пропускается целиком.
        //
        // Ниже оба указателя разыменовываются ШЕСТЬ раз подряд (пружина, затухание, трение, флаги)
        // без единой проверки. А номер материала для треугольной геометрии берётся из
        // `surface.mode`, то есть приходит ИЗ ДАННЫХ УРОВНЯ, а не из кода — негодное значение туда
        // попадает штатным путём.
        //
        // Раньше `GetMaterialByIdx` на негодном номере читал за границами вектора и отдавал мусор:
        // пружина и трение считались по случайным числам, и физика вела себя необъяснимо, не падая.
        // Теперь оттуда приходит честный ноль (см. разбор там же), и его надо обработать.
        //
        // Пропуск контакта — верный ответ: свойства поверхности неизвестны, а выдумывать их значит
        // менять физику по случайным данным. Пара геометрий просто не сталкивается в этом кадре.
        //
        // Найдено разбором журнала Monolith («Possible fix of Physics.cpp: CollideIntoGroup crash»);
        // они пометили правку как «possible», причина проверена по своему коду.
        if (!material_1 || !material_2)
        {
            static u32 da_hits = 0;
            ++da_hits;
            if (da_hits <= 5 || (da_hits % 500) == 0)
                Msg("! [DA] столкновение: материал не найден (%u / %u) — контакт пропущен (случаев %u)",
                    (u32)material_idx_1, (u32)material_idx_2, da_hits);
            continue;
        }
