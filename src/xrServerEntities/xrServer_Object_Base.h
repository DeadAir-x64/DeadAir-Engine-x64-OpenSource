...
public:
    // [DA_PORT] Метка жизни серверной сущности.
    //
    // По указателю нельзя узнать, жив ли объект: чтобы спросить, нужно к нему обратиться, а
    // обращение к удалённому и есть падение. Метка разрывает круг — она ставится в конструкторе и
    // гасится в деструкторе, поэтому «живой» отличим от «удалённого» ДО первого виртуального вызова.
    //
    // Зачем это здесь, в самом основании: реестры ALife (объектов, графа, уровня) держат сырые
    // указатели `CSE_ALifeDynamicObject*`, а он наследует CSE_Abstract. Одна метка в базе закрывает
    // их все разом, вместо проверки в каждой из восьми десятков точек доступа. Расписание живёт в
    // другой ветке иерархии (CSE_ALifeSchedulable), там метка своя, но смысл тот же.
    //
    // ⚠️ Это проверка ВЕРОЯТНОСТНАЯ, а не гарантия: читается память, уже возвращённая аллокатору.
    // Работает потому, что страница ещё отображена процессу; если её успели переписать — число не
    // совпадёт, и мы поступим правильно (сочтём объект мёртвым). Ошибиться в опасную сторону она
    // может только если чужие данные случайно легли ровно этим числом — оттого оно и приметное.
    enum : u32 { da_alive_magic = 0xA11FE5AFu };
    u32 m_da_alive = 0;

    bool da_object_alive() const { return m_da_alive == u32(da_alive_magic); }

    // [DA_PORT] СТРОГАЯ проверка живости, в дополнение к вероятностной метке выше.
    //
    // Отличие принципиальное: метка — это поле ВНУТРИ объекта, и чтобы её прочитать, надо
    // разыменовать указатель, который может быть уже освобождён. Пока страница цела, метка отвечает
    // верно; но как только аллокатор отдал блок кому-то другому, там лежат чужие данные, и ответ
    // становится случайным. Именно в этот момент и происходит краш, ради которого проверка заводится
    // (переход уровня освобождает и раздаёт память сразу).
    //
    // Реестр мёртвый объект не трогает вовсе: он отвечает по самому значению указателя. Поэтому
    // ИМЕННО ОН стоит на скриптовой границе, где Lua держит сырые указатели и ничто их не гасит.
    // Метка остаётся для горячих внутренних обходов, где восемьдесят точек доступа, а реестр был бы
    // дорог.
    //
    // Перенесено по смыслу из Dead-Air-Refined (MMadmer, MIT), где этим закрыт краш в
    // CSE_ALifeOnlineOfflineGroup::commander_id после перехода уровня.
...
// [DA_PORT] Спросить ПЕРЕД обращением через указатель, пришедший из скрипта. Возвращает false, если
// объект уже освобождён, и один раз печатает отказ со стеком скрипта. Разбор — у is_live_object;
// реализация в xrServer_Objects_script.cpp, общая с обращениями к отрядам.
