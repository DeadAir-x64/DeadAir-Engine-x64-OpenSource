...
// [DA_PORT] wpn_state здесь — это CHUDState::EHudStates, а его нумерация возвращена к оригинальной
// (см. HudItem.h). Сейвы прежних сборок несут старые числа, и состояния 0..3 читаются со сдвигом.
//
// ⚠️ Версия сейва под это НЕ НУЖНА, и это проверено, а не предположено: значение никем не
// используется как состояние. Единственный читатель — CWeapon::net_Import, и он смотрит только
// case eFire / eFire2 / eSwitch / eReload, то есть значения 5 и выше, одинаковые в обеих нумерациях.
// Любое 0..3 при любом сдвиге попадает в default и ведёт себя одинаково.
//
// Сначала я записал сюда долг «поднять SPAWN_VERSION 129 -> 130 и пересчитать по таблице». Он был
// написан из осторожности, по факту сериализации, без проверки читателей — и оказался лишним.
