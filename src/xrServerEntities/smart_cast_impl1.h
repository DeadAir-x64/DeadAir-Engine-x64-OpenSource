...
#else
        // GCC rejects explicit template specialization in non-namespace scope; use if constexpr instead
        if constexpr (base)
            return static_cast<T1*>(p);
        else
...
{
    return (CHelper1<T1, T2>::template smart_cast < object_type_traits::is_base_and_derived<T1, T2>::value ||
...
IC T1 smart_cast(T2* p)
{
    ...
    return (SmartDynamicCast::CHelper2<T2>::template smart_cast<std::remove_pointer_t<T1>>(p));
}
...
#endif
    return (*SmartDynamicCast::CHelper2<T2>::template smart_cast<std::remove_reference_t<T1>>(&p));
}
