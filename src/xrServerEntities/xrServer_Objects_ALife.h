...
public:
    ...
    // [DA_PORT] Метка жизни: ставится в конструкторе, гасится в деструкторе.
    //
    // Планировщик ALife обходит расписание по УКАЗАТЕЛЯМ, и висячая запись там означает
    // виртуальный вызов по освобождённой памяти. Все точечные защиты, что мы ставили, шли от
    // конкретного пути удаления — снятие в unregister_object, сверка личности по реестру, — и
    // каждая закрывала свой путь, а падение возвращалось. Метка не зависит от пути вовсе:
    // объект разрушен любым способом — значение больше не совпадает.
    //
    // Чтение освобождённой памяти формально некорректно, но здесь это единственный способ
    // отличить живой объект от мёртвого, и он безопаснее альтернативы: страница ещё
    // отображена (аллокатор её не возвращает системе сразу), а если её успели переписать —
    // метка не сойдётся, и мы сделаем ровно то, что нужно: пропустим запись.
    enum : u32 { da_alive_magic = 0xA11FE5AFu };
    u32 m_da_alive = 0;

    bool da_schedulable_alive() const { return m_da_alive == u32(da_alive_magic); }
