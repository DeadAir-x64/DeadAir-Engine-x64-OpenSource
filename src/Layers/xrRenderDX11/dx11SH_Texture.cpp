...
void CTexture::Unload()
{
    ...
    // [DA_PORT] Основное представление текстуры (m_pSRView) здесь не освобождалось ВООБЩЕ, хотя
    // создаётся оно тут же, в четырёх местах через CreateShaderResourceView(..., &m_pSRView).
    // Освобождались только pSurface, srv_all, srv_per_slice и покадровые представления анимаций.
    //
    // Цена: живое представление держит свою текстуру, поэтому за выгрузкой уровня уходил не один
    // объект, а пара. Перепись живых объектов D3D на выходе (слой отладки, r__d3d_debug 1) показала
    // 487 ID3D11ShaderResourceView с внешней ссылкой и 458 ID3D11Texture2D, у которых собственных
    // ссылок ноль, а внутренняя единица — то есть их держали именно представления. Это ~740
