...
void render_sun::init()
{
    ...
    o.active = ps_r2_ls_flags.test(R2FLAG_SUN) &&
        (u_diffuse2s_nofloor(sun_color.r, sun_color.g, sun_color.b) > EPS);
    ...
    const auto render_cascade = [&, this](const TaskRange<u32>& range)
    {
        for (u32 cascade_ind = range.begin(); cascade_ind != range.end(); ++cascade_ind)
        {
            ...
            if (bNormal || bSpecial)
            {
                ...
                //
                // [DA_PORT] Трава идёт в теневую карту ТОЛЬКО в ближних каскадах — ручка
                // r__grass_shadow_cascades (по умолчанию 1, то есть один ближний).
                //
                // ЗАМЕР (Кордон, обзор на открытый склон, 201 кадр, медианы): с травой во всех трёх
                // каскадах фаза sun_smap стоит 5.05 мс из 8.52 мс всего кадра видеокарты; без травы
                // в тенях — 0.59 мс и 6.14 мс. То есть тени травы съедали 4.46 мс, больше половины
                // теневой фазы и 28% кадра видеокарты. Причина проста: при r__detail_radius 299 вся
                // трава на трёхстах метрах рисуется в КАЖДЫЙ каскад, то есть трижды.
                //
                // Почему именно ближний, а не выключить совсем: вблизи тень от травы — заметная часть
                // картинки, и выключение видно сразу. Каскады у нас 20, 40 и 160 метров; на сорока
                // тень отдельной травинки уже неразличима, на ста шестидесяти — тем более. Платим за
                // то, чего не видно.
                //
                // ⚠️ Ограничение по КАСКАДУ, а не по расстоянию, намеренно: каскад — это готовая
                // граница, по которой проход и так разделён, и она уже согласована с дальностью
                // теней. Отдельная дистанция дала бы вторую границу рядом с существующей, и они
                // разошлись бы при первой же правке каскадов.
                // ⚠️ Объявление ЛОКАЛЬНОЕ и ВНУТРИ пространства имён — как у соседнего
                // ps_r__grass_spot_shadows в r2_R_lights.cpp. Ловушка тут работает в ОБЕ стороны:
                // переменные движка (xrEngine) объявляются снаружи, а переменные рендера
                // (xrRender_console.cpp) живут внутри xray::render::RENDER_NAMESPACE, и объявление
                // снаружи для них не слинкуется.
                extern int ps_r__grass_shadow_cascades;
                if (ps_r2_ls_flags.test(R2FLAG_SUN_DETAILS) && RImplementation.Details &&
                    cascade_ind < u32(ps_r__grass_shadow_cascades))
