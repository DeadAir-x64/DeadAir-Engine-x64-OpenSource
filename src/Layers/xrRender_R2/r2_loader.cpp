...
void CRender::LoadBuffers(CStreamReader* base_fs, bool alternative)
{
    ...
    // Vertex buffers
    {
        ...
        // [DA_PORT] Разведка форматов вершин уровня. Осталась от разбора выборки вершин из шейдера
        // (тот приём отвергнут замером, см. 05_ROADMAP.md), но сама справка полезна и стоит один
        // проход по объявлениям на загрузке: она сразу говорит, сколько в геометрии уровня разных
        // раскладок и какого они размера. Гадать по размеру вершины нельзя — шейдер читает Nh/T/B
        // как float4, а лежат они упакованными по четыре байта.
        {
            ...
            Msg("* [DA_PORT] геометрия уровня: буферов вершин %u, разных форматов %u%s", count,
                u32(unique_decls.size()), alternative ? " (урезанный поток)" : "");
