...
bool CRenderTarget::enable_scissor(light* L) // true if intersects near plane
{
	...
	Irect		R;
	R.x1		= clampr	(iFloor	(bb.min.x*Device.dwRenderWidth),	int(0),int(Device.dwRenderWidth));
	R.x2		= clampr	(iCeil	(bb.max.x*Device.dwRenderWidth),	int(0),int(Device.dwRenderWidth));
	R.y1		= clampr	(iFloor	(bb.min.y*Device.dwRenderHeight),	int(0),int(Device.dwRenderHeight));
	R.y2		= clampr	(iCeil	(bb.max.y*Device.dwRenderHeight),	int(0),int(Device.dwRenderHeight));
	if	( (Device.dwRenderWidth==u32(R.right - R.left)) && (Device.dwRenderHeight==u32(R.bottom-R.top)) )
	{
