...
void CRender::Render()
{
    ...
    // Update incremental shadowmap-visibility solver
    {
        ...
        for (it = 0; it < Lights_LastFrame.size(); it++)
        {
            ...
            try
            {
                // [DA_PORT] УТЕЧКА: сброс шёл по трём слотам, а выдаются четыре.
                //
                // svis[] заведён на R__NUM_CONTEXTS (5), запросы видимости раздаёт alloc_context —
                // а он выдаёт batch_id из 0..R__NUM_PARALLEL_CONTEXTS-1, то есть ЧЕТЫРЕ. Слот 3
                // каждый кадр начинал occlusion-запрос и не забирал его никогда: слот навсегда
                // оставался занятым, пул пустел, и R_occlusion::occq_begin создавал новый запрос
                // D3D на каждую лампу каждый кадр. Замер 18.08: +7 МБ/с ДАЖЕ ПРИ СВЁРНУТОМ ОКНЕ
                // (кадры идут), 48 ГБ за сеанс и падение машины. В кучах Windows этого не видно —
                // объекты живут в памяти драйвера.
