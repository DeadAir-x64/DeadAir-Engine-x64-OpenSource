...
public:
    ...
    // [DA_PORT] Вернуть ячейку в атлас.
    //
    // Половина без пары: занять место было чем, освободить — нечем. Размер карты лампы (L->X.S.size)
    // меняется с дистанцией, и на каждое изменение лампа брала НОВУЮ ячейку, а прежняя оставалась
    // занятой навсегда. Атлас забивался тем быстрее, чем больше игрок ходит, упирался в нехватку
    // места и перекладывался целиком — а перекладка поднимает поколение, то есть обнуляет ВЕСЬ кэш
    // статики и заставляет перерисовать тени всех ламп. Кэш периодически терял смысл сам собой.
    //
    // Слияния соседних свободных прямоугольников здесь нет намеренно: место освобождает та же
    // лампа, которая тут же берёт другое, а общую фрагментацию по-прежнему снимает перекладка.
    // Критические точки не трогаем — они остаются кандидатами на размещение и после освобождения.
    void release(const SMAP_Rect& R)
    {
        for (auto it = stack.begin(); it != stack.end(); ++it)
        {
            if (it->min.x == R.min.x && it->min.y == R.min.y && it->max.x == R.max.x &&
                it->max.y == R.max.y)
            {
                stack.erase(it);
                return;
