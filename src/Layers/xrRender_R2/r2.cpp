...
{
#if RENDER == R_R4
    // [DA_PORT] У create() не было пары, и 52 объекта запросов D3D11 (4 кольца по 13) не отпускались
    // НИКОГДА. Держали ссылки на устройство и уезжали на разрушение вместе с ним — а освобождение
    // ресурсов D3D после того, как устройство ушло, роняет драйвер: тот же почерк, что был у ~da_dlss.
    // Вылет при этом происходит ПОСЛЕ выхода из игры, поэтому выглядит безобидно и живёт долго.
    g_da_gpu_timer.destroy();
#endif
#if defined(USE_DX11)
