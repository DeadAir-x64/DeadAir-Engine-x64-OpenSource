...
// [DA_PORT] КОЛЕБАНИЕ ТРАВЫ ПО МЕСТНОСТИ. 0 - выключено.
//
// Задача. Вдали трава читается ровным ковром одного цвета: тысячи одинаковых кустиков с одной
// текстурой сливаются в заливку. Земля под ними уже колеблется (ps_da_macro_var), а трава нет,
// и оттого они выглядят двумя разными слоями, а не одним покровом.
//
// Приём: яркость кустика зависит от его МИРОВОЙ позиции через шум. Мировой, а не экранный —
// принципиально: узор стоит на месте при движении камеры, как настоящая неровность почвы.
// Экранный шум ехал бы за головой и читался бы грязью на стекле.
//
// ⚠️ Меняется ЯРКОСТЬ, а не цвет. Настоящее согласование цвета требует цветовой карты ландшафта,
// сэмплируемой по мировой позиции (так это делают в отрасли) — у нас такой карты нет, и завести
// её отдельная работа. Яркость даёт большую часть выигрыша даром.
float ps_r__grass_tint = 0.f;

// [DA_PORT] Размер пятна колебания в метрах. Мелкое читается шумом, крупное - неровностью почвы.
float ps_r__grass_tint_scale = 12.f;

// [DA_PORT] Насколько сильнее колебание У ОСНОВАНИЯ стебля, чем у верхушки. 0 - равномерно.
//
// Отраслевой приём смешивания травы с землёй: у основания трава берёт свойства почвы, к верхушке
// остаётся собой. Так стык перестаёт читаться линией — низ темнеет вместе с землёй, верх нет.
float ps_r__grass_tint_base = 1.f;  // [DA_PORT] подобрано в игре 27.08
...
void xrRender_initconsole()
{
    ...
    CMD4(CCC_Float, "r__grass_tint", &ps_r__grass_tint, 0.f, 1.f);               // [DA_PORT]
    CMD4(CCC_Float, "r__grass_tint_scale", &ps_r__grass_tint_scale, 1.f, 80.f);  // [DA_PORT]
    CMD4(CCC_Float, "r__grass_tint_base", &ps_r__grass_tint_base, 0.f, 2.f);     // [DA_PORT]
