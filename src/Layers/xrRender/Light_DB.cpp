...
{
    // ⛔ [DA_PORT] Лампа с НУЛЕВОЙ дальностью не светит ничем, но идёт через весь конвейер, и её
    // проекция при этом ВЫРОЖДЕНА.
    //
    // Считаем по нашим же числам. Матрица тени строится так
    // (Light_Render_Direct_ComputeXFS.cpp):
    //
    //     build_projection(cone + tan_shift, 1.f, L->virtual_size, L->range + EPS_S);
    //                                             ^ ближняя = 0.1   ^ дальняя = range + EPS_S
    //
    // При range == 0 дальняя плоскость оказывается БЛИЖЕ ближней: 1e-5 против 0.1. В построении
    // проекции это даёт Q = f/(f-n) с отрицательным знаменателем, то есть вывернутое отображение
    // глубины. Дальше по этой матрице строится ещё и пирамида отсечения
    // (view_frustum.CreateFromMatrix), и теневой проход рисуется по бессмыслице.
    //
    // Выбрасываем на СБОРЕ, а не в проходе: так отсекаются разом все пути — и теневой, и накопление,
    // и подсчёт бюджета ламп.
    //
    // 🩺 Прибор при защите намеренно: у нас лампа гасится через set_active(false), и неактивная в
    // сбор не попадает — то есть теоретически сюда доходить нечему. Но дальность приходит из
    // скриптов (CTorch::TorchSetRange), и «теоретически» тут не довод. Пусть лог скажет, случается
    // это в игре или нет.
    if (L->range <= EPS_L)
    {
        static u32 da_dropped = 0;
        if (++da_dropped <= 5)
            Msg("~ [DA_PORT] лампа с нулевой дальностью отброшена на сборе: тип %d, тень %d, "
                "дальность %.6f (случаев %u)",
                int(L->flags.type), L->flags.bShadow ? 1 : 0, L->range, da_dropped);
        return;
    }
