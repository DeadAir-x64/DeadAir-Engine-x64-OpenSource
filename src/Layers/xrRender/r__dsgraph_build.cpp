...
void R_dsgraph_structure::insert_dynamic(IRenderable* root, dxRender_Visual* pVisual, Fmatrix& xform, Fvector& Center)
{
    ...
    // HUD rendering
    //
    // [DA_PORT] Ветку решает ФАЗА, а не только флаг. Разбор:
    //
    // mapHUD и mapHUDSorted опустошает ТОЛЬКО render_hud, а его зовёт единственно основной проход —
    // теневые идут через render_graph и render_sorted. Геометрия, попавшая сюда в теневом проходе,
    // молча пропадает и стирается ближайшим сбросом.
    //
    // А renderable_HUD — это простой bool НА ОБЪЕКТЕ, общий для всех контекстов рендера.
    // CHUDManager::Render_Last поднимает его на время отрисовки интерфейса основного прохода и
    // опускает обратно, тогда как задачи каскадов солнца и отбора дождя идут рядом. Задача,
    // прочитавшая флаг внутри этого окна, теряет тень игрока на кадр — она пропадает и сама
    // возвращается, потому что все каскады отбираются заново каждый кадр.
    //
    // Проверка по фазе снимает зависимость от этого совпадения целиком и бесплатно: в теневом
    // проходе класть в mapHUD НЕЛЬЗЯ ни при каких обстоятельствах, флаг там значения не имеет.
    //
    // Найдено сверкой с Dead-Air-Refined (b24cf7bc).
    if (o.phase == CRender::PHASE_NORMAL && root && root->renderable_HUD())
    {
