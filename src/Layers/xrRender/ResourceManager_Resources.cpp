...
CTexture* CResourceManager::_CreateTexture(LPCSTR _Name)
{
    ...
    // [DA_PORT] Пустое имя текстуры — это ДАННЫЕ, а не дефект движка, и ронять из-за них игру нельзя.
    //
    // Найдено 18.08.2026 на чистой установке со сторонним набором текстур CoP 2K: загрузка l05_bar
    // вставала фаталом «_Name && _Name[0]». Механизм — в fix_texture_name: оно обрезает имя по
    // расширению, поэтому запись «#.dds» превращается в «#» (в логе рядом: «Can't find texture '#'»),
    // а запись из ОДНОГО расширения — в пустую строку, которая и приходила сюда.
    //
    // Возврат nullptr здесь не новость для вызывающих: строкой выше функция уже так отвечает на имя
    // «null», то есть пустая ссылка на текстуру — часть её договора, и точки использования её
    // переживают. Соседний путь texture_load ту же проверку давно делает мягко (R_ASSERT1_CURE) —
    // здесь остался незакрытый близнец.
    //
    // Сообщение с ограничителем: битая запись повторяется на каждом материале, который её тянет, и
    // без ограничителя лог заливает одинаковыми строками.
    if (!_Name || !_Name[0])
    {
        static u32 da_empty_name = 0;
        if (++da_empty_name <= 5)
            Msg("! [DA_PORT] запрошена текстура с ПУСТЫМ именем — пропускаю. Причина в данных: "
                "имя из одного расширения либо битая ссылка в .thm стороннего набора текстур");
        return nullptr;
    }
    string_path Name;
