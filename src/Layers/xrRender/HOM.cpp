...
// [DA_PORT] Ручка для чистого теста: r__hom 0 выключает программное отсечение целиком.
//
// Гадать, оно ли гасит объект, бессмысленно — быстрее выключить и посмотреть. Если мигание уходит,
// виновато отсечение; если остаётся, причина в другом месте кадра, и искать надо там.
int ps_da_hom_enabled = 1;

BOOL CHOM::visible(vis_data& vis) const
{
    if (!ps_da_hom_enabled)
        return TRUE;
    ...
    const BOOL raw_result = _visible(vis.box, m_xform_01);

    // [DA_PORT] Гасим объект только после нескольких отказов подряд.
    //
    // Расписание здесь несимметричное, и это главное: «видно» откладывает следующую проверку на
    // 10-25 кадров, «не видно» назначает её на следующий же кадр. Объект, стоящий на границе решения
    // растеризатора, из-за этого гаснет мгновенно, а загорается пачкой на десяток кадров — глаз
    // читает это как мигание целиком, а не как дрожание края.
    //
    // Больше всего страдает вода: это большая плоская поверхность, её габаритный ящик почти лежит в
    // плоскости, и вердикт по нему легко переворачивается от кадра к кадру. Отсюда «мигает всё
    // озеро».
    //
    // Лечение то же, что мы применили к видимости источников света: загораться сразу, гаснуть после
    // трёх отказов подряд. Ошибочный отказ съедается, настоящее исчезновение задерживается на два
    // кадра - незаметно.
    BOOL result = raw_result;
    if (raw_result)
        vis.hom_miss_streak = 0;
    else
    {
        if (vis.hom_miss_streak < 255)
            ++vis.hom_miss_streak;

        if (vis.hom_miss_streak < 3)
            result = TRUE; // ещё не верим
    }
    ...
    if (result)
    {
        ...
        // Пока держим объект на веру, проверять надо каждый кадр: иначе ошибочный отказ утонет в
        // десятке кадров ожидания и гистерезис превратится в задержку.
        if (!raw_result)
            delay = 1;
    }
    else
