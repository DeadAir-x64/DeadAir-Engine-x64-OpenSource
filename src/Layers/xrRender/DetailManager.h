...
public:
    ...
    // [DA_PORT] Признак «задача расчёта работает». Ставится при постановке в очередь, снимается
    // самой задачей на любом выходе. Нужен потому, что указателя на задачу для ожидания мало:
    // он хранит только последнюю, а поставлено их может быть несколько.
    std::atomic<bool> m_calc_active{ false };
