...
private:
    ...
    ref_shader s_water_depth; // [DA_PORT] глубина воды для нашей TAA — см. phase_water_depth
    ref_shader s_frame_diff; // [DA_PORT] прибор разницы кадров — см. phase_frame_diff
    ref_rt rt_Diff_prev;     // [DA_PORT] кадр, каким он был на прошлом кадре
    ref_rt rt_Diff_out;      // [DA_PORT] куда рисуется разница
...
public:
    ...
    void phase_water_depth(); // [DA_PORT] глубина воды для нашей темпоралки
    void phase_frame_diff(); // [DA_PORT] прибор: что дрожит между кадрами
    ...
    void da_light_watch();
    void da_light_map(); // [DA_PORT] карта света по всему экрану, см. da_light_map
    void da_map_capture_gbuffer(); // [DA_PORT] снять альбедо, пока rt_Color ещё альбедо
    void da_map_capture_lighting(); // [DA_PORT] снять свет и трафарет до сборки кадра
    void da_map_capture_combine1(); // [DA_PORT] снять кадр сразу после первого прохода сборки // [DA_PORT] покадровое наблюдение за светом в пикселе, см. .cpp
