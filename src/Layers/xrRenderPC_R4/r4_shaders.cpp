    ...
    if (SUCCEEDED(_hr) && pReflection)
    {
        ...
        if constexpr (std::is_same_v<T, SVS>)
        {
            if (RImplementation.o.velocity)
            {
                ...
                const bool exempt_by_design = file_name &&
                    (strstr(file_name, "shadow_") || strstr(file_name, "accum_") ||
                     strstr(file_name, "lplanes") || strstr(file_name, "stub_") ||
                     strstr(file_name, "dumb"));

                // [DA_PORT] Проверено ГЛАЗАМИ 09.08.2026, разницы не видно — сообщение снято.
                //
                // Основание у этих четырёх другое, чем у списка выше: там джиттер не нужен по
                // существу, а здесь он был бы уместен, но выигрыша не даёт.
                //   sky2, clouds, effects_sun — небо и солнце. Небо уже пишет векторы движения
                //     (da_sky_velocity), то есть перепроекция работает; не хватает лишь
                //     субпиксельного разнообразия. Сравнение с апскейлером и без него разницы не
                //     показало, а джиттер на бесконечности дважды давал у нас ДРОЖАНИЕ вместо
                //     сглаживания (мерцание тени и воды) — риск перевешивает.
                //   portal — полупрозрачные квады затухания на границах секторов, цвет умножен на
                //     0.3 и рисуется задом наперёд. Контраста нет, края прячутся за геометрией.
                //
                // ⚠️ Если появится новый апскейлер или изменится работа с небом — снять это
                // исключение и посмотреть заново. Запись держится на осмотре, а не на замере.
                const bool exempt_checked_by_eye = file_name &&
                    (strstr(file_name, "sky") || strstr(file_name, "clouds") ||
                     strstr(file_name, "effects_sun") || strstr(file_name, "portal"));

                const bool exempt = exempt_by_design || exempt_checked_by_eye;
