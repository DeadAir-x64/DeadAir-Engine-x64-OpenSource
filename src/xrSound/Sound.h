...
// [DA_PORT] Длина звука БЕЗ создания объекта звука.
//
// Скриптам она нужна сама по себе: sound_theme создавал sound_object только затем, чтобы спросить
// :length(), и тут же его выбрасывал. Сам разбор заголовка дешёвый - источники кэшируются в
// s_sources, а i_destroy_source вообще ничего не делает, - но каждый такой вызов порождал обёртку
// CSound и userdata в Lua, то есть мусор на каждую произнесённую реплику.
//
// ⛔ СВОБОДНАЯ функция, а НЕ метод ISoundManager. Первая версия была виртуальным методом интерфейса
// и уронила игру: добавление виртуальной функции сдвигает таблицу виртуальных вызовов, и все модули,
// собранные со старым заголовком, начинают попадать не в тот слот. Стек шёл из xrEngine в xrSound.
// Свободная функция таблицу не трогает и пересборки соседей не требует.
//
// Возвращает СЕКУНДЫ; в миллисекунды переводит биндинг, чтобы совпасть с CScriptSound::Length.
XRSOUND_API float xr_sound_length_sec(pcstr fName);
