...
private:
    ...
    void resize()
    {
        ...
        if (nodes)
        {
            // [DA_PORT] Старый массив узлов освобождался сырым xr_free, без деструкторов. Для
            // неPOD-значений это утечка: у mapNormalItems/mapMatrixItems внутри лежит xr_vector,
            // и std::copy выше делает новым узлам ГЛУБОКУЮ копию — буфер старого узла остаётся
            // висеть в куче навсегда.
            //
            // Цена ошибки не разовая: R_dsgraph_structure::reset() зовёт destroy() у всех карт
            // проходов на КАЖДЫЙ кадр (через alloc_context), поэтому карта каждый кадр отрастает
            // с нуля 64 -> 128 -> 256 ... и на каждом шаге роняет весь предыдущий массив.
            // Замером: живые аллокации 877 МБ / 2.2 млн блоков -> 6815 МБ / 61.4 млн блоков,
            // прирост состоял ровно из буферов _NormalItem (16, 32, 64 ... 2048 байт) и
            // _MatrixItem (96, 192, 384).
            //
            // Деструкторы обязаны идти ПОСЛЕ цикла починки left/right выше: ~xr_fixed_map_node
            // обнуляет эти указатели, а цикл их читает.
