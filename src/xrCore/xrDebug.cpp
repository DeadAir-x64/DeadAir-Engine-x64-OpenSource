...
// [DA_PORT] Печать стека Lua при отказе. xrCore про движок скриптов ничего не знает и знать не
// должен, поэтому тот сам оставляет здесь указатель на свою печать (CScriptEngine::init).
XRCORE_API void (*g_da_lua_stack_printer)() = nullptr;
XRCORE_API int (*g_da_lua_full_gc)() = nullptr;
...
LONG WINAPI xrDebug::UnhandledFilter(EXCEPTION_POINTERS* exPtrs)
{
    ...
    if (!ErrorAfterDialog && !strstr(GetCommandLine(), "-no_call_stack_assert"))
    {
        ...
        // указатель, и это разные дефекты.
        {
            ...
            // [DA_PORT] Стек Lua, если падение случилось внутри скрипта.
            //
            // Машинный стек при переходе по нулевому адресу пуст — разматывать нечего. Но если
            // управление в этот момент было в Lua, движок знает, в какой строке какого скрипта: у
            // интерпретатора свой стек кадров, и он цел. Это единственный способ назвать место, когда
            // обычный стек молчит.
            //
            // ⚠️ Только ОДИН раз за всё время. Печать стека Lua сама лезет в интерпретатор, а он в
            // этот момент уже может быть разрушен — тогда она падает, обработчик зовёт её снова, и
            // так по кругу. В логе это выглядело сотнями кадров «[C]» без единого имени файла, то
            // есть диагностика забивала собой ровно то место, ради которого ставилась.
            //
            // 🪤 Печатать ДО машинного стека НЕЛЬЗЯ, и это проверено логом: обработчик успевал
            // написать «стек скрипта на момент отказа:», падал внутри интерпретатора вторично — и
            // лог обрывался на этой строке. Машинного стека не было вовсе, то есть наша же
            // диагностика съедала единственную улику. Поэтому стек Lua печатается ПОСЛЕ машинного,
            // ниже по функции: сначала гарантированное, потом необязательное.
            FlushLog();
        }
        ...
        *exPtrs->ContextRecord = save;
        ...
        // [DA_PORT] Стек Lua — ПОСЛЕ машинного стека и карты модулей.
        //
        // Если управление в момент отказа было в скрипте, у интерпретатора свой стек кадров, и он
        // называет файл и строку там, где машинный стек пуст. Но лезть в интерпретатор опасно: он
        // сам может быть уже разрушен, и тогда печать падает вторично. Поэтому она стоит последней —
        // упадёт, так упадёт на том, что уже записано.
        //
        // Один раз за всё время: повторный заход означал бы, что мы упали внутри собственной
        // диагностики, и печатать её снова нет никакого смысла.
        {
            static bool da_lua_stack_printed = false;
            if (g_da_lua_stack_printer && !da_lua_stack_printed)
            {
                da_lua_stack_printed = true;
                Msg("! [DA_PORT] стек скрипта на момент отказа:");
                FlushLog();
