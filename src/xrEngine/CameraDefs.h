...
enum ECamEffectorType
{
    cefDemo = 0,
    cefAnsel,
    // [DA_PORT] Полный перехват камеры из скрипта: положение и углы задаются целиком, а не
    // добавляются к штатной камере. Нужен паркуру (подтягивание на уступ) и годится для любой
    // сцены, где камерой на время управляет скрипт.
    cefScriptOverride,
    cefNext
};
