...
void CInput::iCapture(IInputReceiver* p)
{
    ...
    // [DA_PORT] Один приёмник — одна запись в стеке. Повторный захват ПЕРЕНОСИТ его наверх, а не
    // добавляет второй раз.
    //
    // Без этого достаточно умереть с открытой консолью, чтобы потерять управление насовсем. Замером
    // поймана ровно такая последовательность: консоль захватила ввод, поверх лёг ролик смерти
    // (CUISequencer), затем консоль захватила ВТОРОЙ раз и оказалась в стеке дважды. Дальше каждое
    // освобождение снимает по одной записи, и лишняя консоль всплывает наверх — а она глотает всё,
    // включая инвентарь и Escape. Со стороны выглядит как намертво зависшая игра, хотя мир жив и
    // симуляция идёт.
    //
    // Освобождение это чинить не может: iRelease снимает ОДНУ запись, а `CConsole::Hide` возвращает
    // ввод один раз — вторая запись остаётся навсегда. Поэтому дубликат надо не допускать.
    //
    // Перенос наверх, а не отказ от захвата: тот, кто захватывает, рассчитывает получить ввод, и
    // просто проигнорировать его вызов означало бы отдать управление кому-то другому.
    if (const auto it = std::find(cbStack.begin(), cbStack.end(), p); it != cbStack.end())
    {
        Msg("~ [DA_PORT] ввод: приёмник %s захватывает повторно — переносим наверх, дубликат не создаём",
            typeid(*p).name());
        cbStack.erase(it);
    }
    ...
    else
    {
        ...
        for (size_t cnt = cbStack.size(); cnt > 0; --cnt)
            if (cbStack[cnt - 1] == p)
            {
                ...
                cbStack.erase(it);
                // [DA_PORT] Раньше эта ветка молчала, и в логе выглядело так, будто захватов больше,
                // чем освобождений. Изъятие из середины владельца ввода не меняет, но в разборе
                // «кто наверху» без него не сходились цифры.
                da_dump_ir_stack("снят из середины", cbStack);
                break;
            }
    }
}
