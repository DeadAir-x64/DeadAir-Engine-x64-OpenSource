...
{
    // [DA_PORT] Сюда приходит строка ИЗ СКРИПТА: `CUICustomEdit::SetText` экспортирован в Lua, а
    // luabind отдаёт на месте nil обычный нулевой указатель. В моде такой вызов живой — в меню
    // настроек `axr_main_options` текст поля берётся из конфигурации (`config:r_value`), и на
    // отсутствующем ключе с пустым значением по умолчанию приходит nil.
    //
    // ⭐ Соседняя проверка: `CUILines::SetText` ноль обрабатывает (`if (text && text[0] != 0)`).
    // Не обрабатывал только этот путь. Правка взята из Anomaly 1.5.2.
    if (!str)
        str = "";
...
void remove_spaces(pstr str)
{
    ...
    // [DA_PORT] Срезаем ХВОСТОВОЙ пробел. Без этого функция, названная remove_spaces, схлопывает
    // подряд идущие пробелы, но один в конце оставляет: "r2_sun_details on    " превращается в
    // "r2_sun_details on ", аргумент приходит в команду как "on " и не совпадает ни с чем.
    //
    // Отказ тихий и обвиняет не того: в логе появляется «~ Invalid syntax in call to '<команда>'»,
    // и читается это как неверное значение в конфиге, хотя значение верное — виноват пробел, которого
    // не видно. Поймано на одной строке в rspec_extreme.ltx мода, где настройка травы в солнечных
    // тенях из-за этого не применялась НИ РАЗУ, ни на одном уровне качества.
