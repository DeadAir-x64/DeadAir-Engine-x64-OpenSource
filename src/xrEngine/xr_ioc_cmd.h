...
// [DA_PORT] Крутилка «на один сеанс»: ведёт себя как обычная, но НЕ пишется в user.ltx.
//
// Для диагностики сохранение — не удобство, а мина. Два случая из этого порта:
//
// • `da_sun_only` изолирует один каскад солнечных теней, картинка при этом честно делится надвое —
//   для замера так и надо. Но команда сохранялась как обычная настройка, переживала перезапуск и
//   уехала к игрокам: включил однажды ради замера — и мир «поделён» навсегда. Строки такой в меню
//   нет, в логе ничего, связать причину со следствием игрок не может в принципе.
//
// • Ловушка аллокатора, оставленная взведённой, дважды роняла игру: сохранялась в конфиг, взводилась
//   при следующем запуске и снимала стеки во время загрузки уровня, где память выделяют сразу
//   несколько потоков. Оба раза лог обрывался без стека падения — то есть без намёка на то, что
//   виноват инструмент отладки.
//
// Забыть выключить диагностику слишком легко, а цена ошибки достаётся не тому, кто её включил.
// Правило: диагностика живёт до конца сессии и умирает вместе с ней. Прочитать значение из user.ltx
// команда по-прежнему может (если дописать строку руками) — не пишет туда только сама.
template <class T>
class CCC_DaDebug final : public T
{
public:
    using T::T;
...
using CCC_DaDebugInteger = CCC_DaDebug<CCC_Integer>;
using CCC_DaDebugFloat = CCC_DaDebug<CCC_Float>;
