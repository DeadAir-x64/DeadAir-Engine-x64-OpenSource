...
// [DA_PORT] Пресет модели DLSS: 0 - как решит драйвер, 11 - K, 10 - J, 12 - L, 13 - M.
//
// Полный разбор - у da_ngx_set_preset в Externals/nvngx_shim/da_ngx_api.h. Коротко: пресет выбирает
// МОДЕЛЬ реконструкции, и заголовок NVIDIA прямо связывает выбор модели с мерцанием тонких деталей.
// Ручка живая: смена значения пересоздаёт фичу на ближайшем кадре, перезапуск не нужен.
ENGINE_API int ps_r__dlss_preset = 0;
...
// [DA_PORT] Проход глубины воды для НАШЕЙ темпоралки: 1 — вода пишет свою глубину в G-буфер позиций,
// 0 — проход выключен и там остаётся глубина ДНА (поведение до появления прохода).
//
// Апскейлеру это не нужно и на него не действует: у него та же беда закрыта своими векторами
// (r__water_velocity). Разбор — в r4_rendertarget_phase_water_depth.cpp.
ENGINE_API int ps_da_water_depth = 1;

// [DA_PORT] СВОЙ сдвиг мипов для детальной текстуры, В МИПАХ. Плюс - мягче, минус - резче.
//
// ⚠️ Именно в мипах, а не в сотых долях, как у соседней r__taa_mipbias. Та целочисленная, и сотые
// ей нужны, чтобы задавать дробь; эта float, и делить не за чем. Первая версия делила на 100 в
// связывателе, а предел команды оставила 4.0 - полмипа пришлось бы писать как 50, чего команда не
// принимала. Две записи одной величины в одном коде - готовый способ ошибиться.
//
// Зачем отдельно от общего. Под апскейлером общий сдвиг уходит в минус на log2(рендер/вывод) - это
// штатный спутник любого апскейлера, и у нас он даже консервативнее вендорского: AMD и NVIDIA обе
// советуют log2(...) - 1.0, у нас этой единицы нет. Значит общий сдвиг мерцания НЕ объясняет.
//
// Объясняет его сама детальная текстура: она высокочастотна по замыслу, рисунок на уровне пикселя.
// При рендере 77% выборок меньше, а r__geometry_lod растягивает её вдаль - за Найквистом рисунок
// мерцать обязан. Руководство AMD по FSR 2 предписывает ровно это лечение: править MIP-байас "для
// конкретного высокочастотного текстурного содержимого", а не общей ручкой на все текстуры сразу.
//
// Ноль - как было. Полмипа (0.5) при рендере 77% примерно отменяет для детали общий сдвиг,
// оставив его всему остальному.
ENGINE_API float ps_da_detail_mipbias = -2.f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] Дальность, на которой детальная нормаль ландшафта полностью гаснет, в метрах. 0 - как
// было, то есть не гаснет НИКОГДА.
//
// Зачем. У ландшафта нормаль собирается ЦЕЛИКОМ из четырёх детальных карт (deffer_impl_flat.ps,
// ветка USE_4_BUMP) - вершинная нормаль в неё не подмешивается вовсе.
//
// ⚠️ ПОПРАВКА к прежней редакции этого текста: здесь стояло «детальное альбедо штатно гаснет с
// расстоянием, а нормаль нет». Это НЕВЕРНО. Координата детали считается как
// `O.tcdbump = O.tcdh * dt_params` (deffer_base_bump.vs) - простое масштабирование, и затухания с
// расстоянием нет НИ У АЛЬБЕДО, НИ У НОРМАЛИ. Вдали деталь не исчезает, а сминиферивается в свой
// средний цвет, отчего даль выглядит ровной заливкой.
//
// Для нормали это ничего не меняет: затухание ей всё равно нужно, потому что усреднённая нормаль -
// это не «плоско», а «случайно». Тайлинг у
// этих карт мелкий, и за несколько десятков метров рисунок уходит за Найквист - нормаль начинает
// скакать от пикселя к пикселю, а дрожание апскейлера водит выборку по этой ряби каждый кадр. На
// склоне, где солнце падает вскользь, отклик на такую рябь максимален, и это видно как мерцание
// земли вдали.
//
// Измерено прибором r__motion_vectors 7 (разброс нормали поперёк пикселя): на дальнем плане он
// насыщен, вблизи чёрный. Сдвиг мипов (r__detail_mipbias 4) разброс уменьшает, но не убирает -
// отсюда и лечение не сдвигом, а затуханием: за порогом нормаль обязана сойтись к гладкой
// вершинной, потому что никакой достоверной детали там уже нет.
//
// Гаснет плавно: до половины дальности - в полную силу, дальше линейно до нуля.
ENGINE_API float ps_da_detail_nfade = 0.f;

// [DA_PORT] КРУПНЫЙ СЛОЙ ВАРИАЦИИ для дальнего плана земли. 0 - выключено.
//
// Задача. Вдали деталь не исчезает - она сминиферивается в свой СРЕДНИЙ цвет, и земля становится
// «базовая текстура, умноженная на константу». Повторов при этом не видно (они усредняются), но не
// видно и вообще ничего: даль читается ровной заливкой, а не поверхностью.
//
// Приём отраслевой, в Unreal его зовут macro/micro variation: ту же текстуру берут ЕЩЁ РАЗ с шагом
// повторения в десяток раз крупнее. Мелкий рисунок работает вблизи, крупный не усредняется даже на
// сотнях метров - его период больше, чем накрывает пиксель, - и даль сохраняет крупные светлые и
// тёмные пятна.
//
// Берётся ЯРКОСТЬ крупной выборки и модулирует цвет вокруг единицы: светлее среднего - осветляем,
// темнее - затемняем. Цветовой тон не трогаем, иначе на стыке слоёв поплыл бы оттенок.
//
// Одна дополнительная выборка на пиксель земли, и только за порогом начала - вблизи ветка не
// исполняется вовсе.
ENGINE_API float ps_da_macro_var = 0.5f;       // сила, 0..1  // [DA_PORT] подобрано в игре 27.08
ENGINE_API float ps_da_macro_var_scale = 16.f; // во сколько раз КРУПНЕЕ шаг повторения
ENGINE_API float ps_da_macro_var_start = 40.f; // метры: где вариация начинает нарастать
ENGINE_API float ps_da_macro_var_end = 250.f;  // метры: где выходит на полную силу

// [DA_PORT] «ФАЛЬШИВАЯ ТРАВА»: сила подмены ОТТЕНКА дальней земли на оттенок детали. 0 - выключено.
//
// Задача. Настоящая трава рисуется до r__detail_radius, дальше её нет, и земля остаётся такой, какой
// её задаёт базовая текстура уровня - обычно бурой. Отсюда ощущение, что трава «обрывается»: она не
// обрывается, просто дальше начинается голая земля.
//
// Приём отраслевой: вдали не рисовать траву, а ПОКРАСИТЬ землю в её цвет. Мы берём цвет не «похожий
// зелёный», а собственный цвет детальной текстуры этого места - она и есть трава, и уже сэмплирована
// крупным слоем вариации (ps_da_macro_var), так что новых выборок не требуется.
//
// ⚠️ Тянем ТОЛЬКО оттенок, яркость земли сохраняем. Иначе подмена перебила бы освещение: дальний
// склон в тени стал бы такой же яркости, что и освещённый, и рельеф сплющился бы.
//
// ⛔ Работает вместе с r__macro_var: цвет берётся из его выборки. При r__macro_var 0 не действует.
ENGINE_API float ps_da_macro_tint = 0.6f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] КРУПНЫЙ РЕЛЬЕФ дальней земли. 0 - выключено. Пара к ps_da_macro_tint.
//
// Задача. Цветом мы уже красим даль в оттенок травы, но одного цвета мало: получается КРАШЕНАЯ
// ПЛОСКОСТЬ. Настоящая поверхность вдали опознаётся не рисунком (он всё равно усредняется мипами), а
// игрой света на неровностях — где-то склон повёрнут к солнцу, где-то от него.
//
// Поэтому подмешиваем крупную детальную НОРМАЛЬ тем же приёмом, что и цвет: шаг повторения в
// десяток раз крупнее, чтобы мип его не усреднил. Даль получает светотень и читается поверхностью.
//
// ⚠️ Это НЕ трава и не претендует ею быть. Это дешёвая видимость: ноль объектов, одна дополнительная
// выборка, и та только за порогом дальности. Настоящая трава остаётся в пределах r__detail_radius.
ENGINE_API float ps_da_macro_relief = 0.1f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] СМЕШИВАНИЕ СЛОЁВ ЗЕМЛИ ПО ВЫСОТЕ вместо линейного. 0 - как было.
//
// Задача. У ландшафта четыре детальных слоя, и смешиваются они ЛИНЕЙНО: цвет слоя умножается на его
// вес из маски, результаты складываются. На переходе это даёт либо мыльную полосу, либо ровную
// линию — то, что глаз читает как «границу» и что выглядит хуже любого из слоёв по отдельности.
//
// Отраслевое лечение — height-based splatting: побеждает не тот слой, у которого больше вес, а тот,
// у которого в этой точке ВЫШЕ рельеф. Край получается зубчатым и естественным: камешки проступают
// сквозь песок, трава сквозь грунт. Так делает Enhanced Shaders для Anomaly.
//
// Карт высоты у слоёв нет, поэтому за высоту берём ЯРКОСТЬ самой текстуры — стандартная дешёвая
// замена: светлое считаем выступающим. Дополнительных выборок ноль, все четыре цвета уже прочитаны.
//
// ⚠️ Теми же весами считаются и НОРМАЛИ. Иначе цвет переходил бы по одной границе, а рельеф по
// другой, и стык стал бы виден вдвойне.
ENGINE_API float ps_da_terrain_blend = 1.f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] ДРОЖАНИЕ КООРДИНАТЫ МАСКИ — против видимых границ слоёв ВДАЛИ. 0 - как было.
//
// Чем отличается от r__terrain_blend и почему одного его мало. Смешивание по высоте опирается на
// яркость детальной текстуры, а вдали она сминиферивается в один тон: «высота» у всех четырёх слоёв
// становится одинаковой, и приём вырождается в линейный. Вблизи он работает, вдали не может по
// построению — это проверено в игре.
//
// А вдали видно уже не переход между слоями, а САМУ МАСКУ: это отдельная текстура ландшафта низкого
// разрешения, один тексель на несколько метров. Её плавный градиент, растянутый на десятки метров,
// и читается полосой.
//
// Лечение: сдвигать точку выборки маски на небольшую случайную величину. Граница перестаёт быть
// гладкой дугой и рассыпается на языки. Работает на ЛЮБОЙ дистанции, потому что зависит не от
// детальной текстуры, а от масштаба самого дрожания — он взят крупным, таким, что мип его не съедает.
//
// Шум берётся из уже загруженной детальной текстуры на крупном шаге. Новых ресурсов не нужно, цена —
// одна выборка.
//
// Значение в координатах текстуры ландшафта, поэтому мелкое: начинать с 0.005.
ENGINE_API float ps_da_mask_jitter = 0.005f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] ЧТЕНИЕ ВСЕХ ЧЕТЫРЁХ СЛОЁВ ДЕТАЛИ НА КРУПНОМ ШАГЕ — структура дали. 0 - выключено.
//
// Почему разрешение текстуры тут не при чём (проверено арифметикой). Деталь тайлится примерно раз в
// метр при 2048 текселях, то есть около двух тысяч текселей на метр. На ста метрах один экранный
// пиксель накрывает ~10 см земли, то есть ~200 текселей, и видеокарта обязана их усреднить — берётся
// мип размером порядка 8x8. Эта восьмипиксельная версия ОДИНАКОВА и у 2К, и у 4К, и у 8К: мип-цепочка
// сходится к одному среднему. Повышать разрешение бессмысленно, верхние уровни вдали не читаются.
//
// Побеждает не разрешение, а МАСШТАБ. Читая ту же текстуру с шагом повторения в N раз крупнее, мы
// заставляем пиксель накрывать в N раз меньше текселей — и видеокарта берёт во столько же раз
// подробнее уровень. Узор доживает до горизонта.
//
// Прежние приёмы (ps_da_macro_var, ps_da_macro_tint) брали ОДИН слой и только его яркость либо
// усреднённый цвет смеси — отсюда вдали появлялся тон и пятна, но не появлялась СТРУКТУРА. Здесь
// читаются все четыре слоя и смешиваются ТЕМИ ЖЕ весами, что вблизи, поэтому даль получает настоящую
// детальную текстуру своего покрытия, просто с более крупным узором.
//
// Цена: четыре выборки на пиксель земли, и только за порогом дальности — ближе ветка не исполняется.
ENGINE_API float ps_da_macro_detail = 0.f;  // [DA_PORT] ОТКЛОНЕНО при подборе 27.08:
// крупная деталь дали в игре не понравилась и была выключена. Код оставлен рабочим —
// приём верный по сути, вопрос в подборе масштаба (ps_da_macro_var_scale).

// [DA_PORT] ЩИТЫ ДАЛЬНЕЙ РАСТИТЕЛЬНОСТИ: приведение к виду ближней. Единицы = как было.
//
// Симптом: дальние деревья стоят белёсыми, ближние — нормально зелёными, и на переходе видна полоса.
//
// Причина не в форме щита, а в РАССЕЯННОЙ составляющей. Щит кладёт в G-буфер запечённый hemi
// (`H.w` в lod.ps), снятый при открытом небе, тогда как у ближней геометрии он живой и почти всегда
// меньше — крона сама себя затеняет. Дерево вдали получает полный свет неба и выцветает.
//
// Отраслевой приём здесь тот же: задать импостору те же свойства материала, что у ближней модели.
// Перезапечь мы не можем (щиты лежат в уровнях, их делает xrLC), поэтому подгоняем на выходе.
//
// hemi — главная ручка, две другие вспомогательные: насыщенность вытягивает цвет, потерянный при
// запекании в маленькую текстуру, яркость правит общий уровень, если он уехал.
ENGINE_API float ps_da_lod_hemi = 2.f; // доля рассеянного света щита  // [DA_PORT] подобрано в игре 27.08
ENGINE_API float ps_da_lod_sat = 2.f;  // насыщенность  // [DA_PORT] подобрано в игре 27.08
ENGINE_API float ps_da_lod_bright = 1.f;

// [DA_PORT] Сила хешированного альфа-теста для листвы и травы: 0 - постоянный порог, как было,
// 1 - полностью случайный на пиксель и кадр. Разбор - в shaders/r3/sload.h у da_hashed_aref.
ENGINE_API float ps_da_alpha_hash = 0.f;

// [DA_PORT] ПРИБОР: показать, что на экране меняется от кадра к кадру. 0 - выкл, 1 - серым,
// 2 - цветом (зелёное слабо дрожит, красное сильно). Разбор - в r4_rendertarget_phase_frame_diff.cpp.
ENGINE_API int ps_da_frame_diff = 0;
// Во сколько раз усилить разницу, прежде чем показать. Настоящее мерцание тонкое, без усиления
// картинка была бы почти чёрной.
ENGINE_API float ps_da_frame_diff_gain = 20.f;
...
// [DA_PORT] ПРИБОР ОСВЕЩЕНИЯ: показывает, из какого слагаемого складывается пиксель. 0 - выключен.
//
// Зачем. «Листва белеет на солнце» — симптом, у которого не меньше пяти правдоподобных причин:
// оператор тонировки, зеркальный блик, свет полусферы, множитель солнца, само альбедо. Отличить их
// на глаз нельзя, а перебирать ручками дорого и обманчиво: каждая что-нибудь да меняет, и любое
// изменение легко принять за попадание.
//
// Прибор разбирает сумму на слагаемые и показывает их по отдельности, ПОВЕРХ тумана — чтобы даль
// не подмешивала свой серый и не путала картину.
//
//  1 - только зеркальный блик (spec). Это АДДИТИВНО БЕЛЫЙ член: если белизна тут, тонировка бессильна.
//  2 - всё, КРОМЕ блика. Если тут белизна осталась, блик ни при чём.
//  3 - только свет полусферы, умноженный на альбедо.
//  4 - только солнце и лампы, умноженные на альбедо.
//  5 - чистое альбедо из G-буфера, без всякого света.
//  6 - глянец из G-буфера, серым.
//  7 - множитель ЭКРАННОЙ ТЕНИ (accum_sun_near.ps): белое - свет, чёрное - тень.
ENGINE_API int ps_da_light_probe = 0;

// [DA_PORT] УСИЛИТЕЛЬ ПРИБОРА: во сколько раз умножить показанную величину. 1 - как есть.
//
// Зачем. Слабое слагаемое на тёмном экране неотличимо от нуля, и на этом легко ошибиться в обе
// стороны: назвать нулём то, что просто мало, или разглядеть в шуме то, чего нет. Умножение
// переводит спор «видно или не видно» в измерение: если при десятикратном усилении величина
// ВСЁ ЕЩЁ чёрная, она действительно ноль.
ENGINE_API float ps_da_light_probe_gain = 1.f;

// [DA_PORT] ДЫМКА ИЗ НЕБА (mip fog). Доля, 0 - плоский fog_color, как было.
//
// Задача: граница карты читается серой шторой. Причина не в дальности, а в том, что туман у нас
// красится ОДНИМ цветом на весь кадр: color = lerp(color, fog_color, fog). Дальний холм и небо над
// ним получают одинаковую заливку, и стык остаётся виден при любой плотности.
//
// Приём: брать цвет дымки из кубической карты НЕБА по направлению взгляда, с сильно размытого
// уровня. Тогда мгла в каждой точке горизонта окрашена ровно в то небо, которое за ней — светлее у
// солнца, темнее в противоположной стороне, — и стык исчезает сам, потому что это один и тот же
// цвет. Так сделано в Enhanced Shaders для Anomaly, где приём и называется mip fog.
//
// ⚠️ Не путать с прежним ps_da_fog_sky_tint: тот подмешивал ОДИН усреднённый оттенок неба на весь
// кадр и потому оказался бесполезен (небо у нас серое в 94% погод). Направление он не учитывал
// вовсе, а вся сила именно в нём.
//
// Даром: карты sky_s0/sky_s1 уже привязаны в hmodel.h, который combine_1.ps и так включает.
// [DA_PORT] ОБЩАЯ СИЛА НАШЕЙ ДЫМКИ, 0..1. Единица - полностью, ноль - стоковый туман мода.
//
// Одна ручка вместо восьми. Дымка из неба, высотный туман, потолок плотности, приближение по
// дальности видимости - всё это одна возможность с точки зрения игрока, и гасить её он должен
// одним движением, а не перебирая r__fog_* по одной.
//
// ⚠️ ВЕЩЕСТВЕННАЯ, а не целочисленная: ручка выведена ползунком в окне «Настройка прицела
// и цвета», вкладка «Изображение». У ползунка с целочисленной командой движок заполняет
// только целое поле, а вещественное остаётся нулём — на этом в проекте уже обжигались
// (анизотропия). Заодно промежуточные значения дают дымку вполсилы, а не только да/нет.
//
// ⚠️ Гасится ТОЛЬКО наше. Погодный туман мода (fog_near/fog_far из конфигов) остаётся: он часть
// самой погоды, и выключать его нельзя - без него у ясного дня пропадёт даль, а у туманного утра
// исчезнет смысл. Ноль здесь означает «как было до нас», а не «тумана нет».
// [DA_PORT] ЭКРАННЫЕ ТЕНИ: сила. 0 - выключены.
//
// Зачем они при живых теневых картах. Карта солнца кроет десятки метров одним разрешением, и
// мелочь у самой земли в неё не попадает: стебель травы тоньше текселя. Отсюда и жалоба, с
// которой всё началось — низ травы не прилегает к земле, а словно висит над ней.
//
// Экранная тень считается не от карты, а от того, что уже нарисовано, и потому видит именно
// мелочь. Разбор приёма — в da_sss.h.
//
// ⚠️ Ставится ТОЛЬКО в ближний проход солнца. Это эффект контакта; в дальнем каскаде на 160
// метрах луч длиной в треть метра короче пикселя, и платить за него там нечем.
ENGINE_API float ps_da_sss = 0.f;

// [DA_PORT] Длина луча в метрах. Чем длиннее, тем дальше тянется тень — и тем дороже.
//
// Держать коротким: приём видит только то, что НА ЭКРАНЕ, и у края кадра луч выходит за него,
// обрывая тень. Чем длиннее луч, тем шире полоса у краёв, где тень пропадает.
ENGINE_API float ps_da_sss_len = 0.35f;

// [DA_PORT] Предполагаемая толщина преграды, метры.
//
// Буфер глубины плоский и не знает, насколько предмет толстый. Разница глубин больше этого числа
// считается «луч прошёл ЗА объектом», а не «упёрся в него». Без верхнего порога любая преграда
// была бы бесконечно глубокой, и тень тянулась бы от неё через всю сцену.
ENGINE_API float ps_da_sss_thick = 0.5f;

// [DA_PORT] Число шагов луча. Прямая цена: столько выборок глубины на пиксель.
ENGINE_API float ps_da_sss_steps = 8.f;

ENGINE_API float ps_da_fog = 1.f;

// [DA_PORT] ЦВЕТ СОЛНЕЧНЫХ ЛУЧЕЙ ОТ НЕБА У ГОРИЗОНТА, 0..1. 0 - как было.
//
// Замечание автора мода: объёмные лучи ЖЕЛТЯТ, и на дистанции без преград ведут себя как туман.
// Проверено в шейдере — так и есть. sunshaftsdisplay.ps берёт цвет ПОЛУСФЕРЫ, удваивает
// насыщенность и НОРМАЛИЗУЕТ вектор: от неба остаётся чистый оттенок на полную силу, независимо
// от его настоящей яркости. Любой тёплый подтон превращается в жёлтый.
//
// А наш туман красится небом У ГОРИЗОНТА (ps_da_fog_sky). Два источника тянут в разные стороны,
// и на дистанции спорят: мгла холодная, лучи жёлтые.
//
// Главное правило тумана, названное автором: он обязан совпадать с небом на горизонте. Раз лучи
// вдали ведут себя как туман, правило относится и к ним. Эта ручка сводит их к одному источнику.
ENGINE_API float ps_da_shafts_sky = 0.f;

ENGINE_API float ps_da_fog_sky = 0.8f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] Размытость неба для дымки: номер уровня мип-цепочки. Мелкие облака не должны проступать
// в тумане — нужен общий тон направления, а не рисунок.
ENGINE_API float ps_da_fog_sky_mip = 6.f;

// [DA_PORT] ПРИЖАТИЕ выборки неба к горизонту, 0..1. 1 - брать строго горизонтальный цвет.
//
// 🪤 Найдено на закате. Первая версия брала небо ПО НАПРАВЛЕНИЮ ВЗГЛЯДА НА ПИКСЕЛЬ — и у высокого
// дерева это направление смотрит ВВЕРХ, так что верхушка красилась цветом неба над собой. На
// закате вертикальный градиент огромен, и дальние сосны выходили СВЕТЛЕЕ мглы вокруг них на той же
// дистанции. Такого быть не может: они в одном и том же воздухе.
//
// Физически рассеяние идёт вдоль луча ДО ОБЪЕКТА, а он почти горизонтален. Значит и цвет должен
// быть горизонтный, независимо от того, насколько высоко предмет забрался в кадре. Прижимаем
// вертикальную составляющую направления перед выборкой.
//
// Днём разницы почти нет — небо ровное, и ошибка не проявлялась вовсе. Ещё один случай, когда
// дефект живёт только в одном режиме и на глаз в остальных не виден.
ENGINE_API float ps_da_fog_sky_flat = 0.8f;

// [DA_PORT] ВЫСОТНЫЙ ТУМАН: сила. 0 - выключен.
//
// Плотность падает с высотой по экспоненте: низины и подножия заливает, гребни и небо остаются
// чистыми. Именно это превращает штору в воздух — дальний холм тонет снизу вверх, появляется
// слоистость. Интеграл вдоль луча берётся аналитически (классика Crytek), лишних выборок нет.
// ⚠️ ЕДИНИЦА ЗДЕСЬ — рабочая величина, а не предел. В шейдер уходит поделённой на тысячу:
// плотность в этой формуле считается НА МЕТР ПУТИ, и разумные значения там — тысячные
// доли. Первая версия отдавала число как есть, показатель экспоненты уходил за сотню,
// и на единице весь кадр превращался в молоко.
ENGINE_API float ps_da_fog_height = 0.5f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] Высотный туман: скорость падения с высотой. Больше - тоньше слой у земли.
ENGINE_API float ps_da_fog_height_falloff = 0.02f;

// [DA_PORT] ОПОРНАЯ ВЫСОТА слоя дымки, мировые метры. Ниже неё туман густеет, выше — редеет.
//
// 🪤 Без неё падение с высотой почти не работало, и это была моя ошибка: высота отсчитывалась от
// НУЛЕВОЙ ОТМЕТКИ МИРА, а игрок стоит примерно на ней же. Множитель exp(-y*b) выходил около единицы
// при любом b, и ручка falloff выглядела мёртвой, хотя считалась честно.
ENGINE_API float ps_da_fog_height_base = -15.f;  // [DA_PORT] подобрано в игре 27.08

// [DA_PORT] ДАЛЬНОСТЬ ДЫМКИ: множитель на погодные fog_near/fog_far. 1 - как в конфиге погоды.
//
// Чего не хватало. Плотность, слоистость, потолок и цвет уже регулируются, а САМО РАССТОЯНИЕ, с
// которого туман начинается, приходило только из погоды и в консоль выведено не было. Между тем
// это главный рычаг для «границы карты»: отодвинув начало, получаешь чистую середину сцены и мглу
// только у горизонта, вместо ровной пелены с двадцати метров.
//
// Множитель, а не абсолютные метры, намеренно: у 116 погодных файлов свои значения, и подменять их
// одним числом означало бы стереть разницу между ясным днём и туманным утром. Здесь же вся погода
// сдвигается пропорционально и сохраняет характер.
//
// Тем же занят мод Atmospherics для Anomaly — отодвигает туман в ясную и малооблачную погоду.
//
// ⚠️ Действует и на воду (water.ps берёт тот же fog_params), это правильно: иначе вода и берег
// оказались бы в разном тумане.
ENGINE_API float ps_da_fog_dist = 0.5f;  // [DA_PORT] подобрано в игре 27.08 — дымку ПРИДВИНУЛИ, а не отодвинули

// [DA_PORT] ДЫМКА СЛЕДУЕТ ЗА ДАЛЬНОСТЬЮ ВИДИМОСТИ. 0 - не следует, как было.
//
// Зачем. У тумана есть работа помимо красоты: он прячет границу отсечения, переходы уровней
// детализации и подмену дальних объектов на болванки. Кто снижает дальность видимости ради
// кадров, тот от этих переходов и страдает больше всех — а туман у него при этом остаётся
// стоять там же, где у владельца мощной машины, и прятать ему нечего.
//
// Поэтому дальность дымки умножается на ОТНОШЕНИЕ положения ползунка «Дальность видимости»
// (psVisDistance, ходит 0.4..1.5) к точке отсчёта. Опустил дальность вдвое — мгла придвинулась
// вдвое и накрыла ту самую границу, где теперь пропадает геометрия. Поднял — отошла.
//
// ⚠️ ТОЧКА ОТСЧЁТА — 1.5, а не единица. Это положение ползунка, при котором туман подбирали в
// игре, и все умолчания r__fog_* сняты именно оттуда. Возьми за отсчёт единицу — и у того, кто
// подбирал, дымка сама уехала бы в полтора раза дальше подобранного, без единой команды.
//
// ⚠️ Именно ОТНОШЕНИЕ, а не разность. Разность (1 + (vis - ref) * follow) на нижнем краю
// ползунка даёт 1 + (0.4 - 1.5) = -0.1, то есть отрицательную дальность — туман вывернулся бы
// наизнанку. Отношение ведёт себя ровно на всём ходу и никогда не меняет знак.
//
// Меняется только РАССТОЯНИЕ линейной части. Плотность высотного тумана (ps_da_fog_height) к
// дальности видимости отношения не имеет и остаётся как есть: иначе на слабой машине пришлось бы
// не только придвинуть мглу, но и сгустить её, а это уже вкус, а не сокрытие переходов.
ENGINE_API float ps_da_fog_follow_vis = 1.f;

// [DA_PORT] ПОТОЛОК плотности дымки. 1 - без потолка, как было.
//
// Даль не должна доходить до сплошной заливки: если оставить немного контраста на горизонте,
// силуэты холмов сохранятся и картинка не станет плоской.
ENGINE_API float ps_da_fog_max = 0.95f;  // [DA_PORT] подобрано в игре 27.08
...
// [DA_PORT] Модуль анимаций — отдельный архив da_animations.xdb0 (движения рук при еде, лечении,
// надевании, подборе предмета, открытии рюкзака, подтягивании на уступ).
//
// Устроено ровно как сезон и по той же причине: архивы монтируются ДО консоли и user.ltx, спросить
// настройку в тот момент негде, поэтому выбор хранится признаком-файлом рядом с архивом. Отсюда же
// и «применится после перезапуска» в подписи: отмонтировать архив посреди сеанса нельзя, на нём
// висят уже загруженные модели и звуки.
//
// ⚠️ Это ОБЩИЙ выключатель модуля, а не набор поштучных. Отдельные анимации гасятся вживую в MCM
// (раздел EA_settings); заводить здесь вторые ручки на те же величины нельзя — две ручки на одну
// переменную перебивают друг друга, и игрок не поймёт, какая победила.
ENGINE_API u32 ps_da_animations = 1;

ENGINE_API xr_token qda_animations_token[] = {
    { "ui_mm_anim_off", 0 },
    { "ui_mm_anim_on", 1 },
    { nullptr, 0 },
};

// Лежит ли архив модуля там, где его ищет файловая система. Проверяем файлом на диске: сам архив в
// виртуальной файловой системе не числится — он ею и является, поэтому FS.exist о нём не знает.
static bool da_animations_archive_present()
{
    string_path path;
    FS.update_path(path, "$arch_dir$", "da_animations.xdb0");
...
class CCC_DaAnimations : public CCC_Token
{
public:
    CCC_DaAnimations(pcstr N, u32* V, const xr_token* T) : CCC_Token(N, V, T) {}

    // В user.ltx не сохраняем: правду хранит признак рядом с архивом, и она нужна раньше, чем
    // user.ltx вообще исполняется. Две записи одного и того же разошлись бы при первом же
    // расхождении, и победила бы та, что применяется позже.
    void Save(IWriter*) override {}
    ...
    void Execute(pcstr args) override
    {
        ...
        string_path marker;
        FS.update_path(marker, "$arch_dir$", "da_animations.txt");

        if (FILE* f = fopen(marker, "wb"))
        {
            const pcstr value = (0 == ps_da_animations) ? "off" : "on";
            fwrite(value, 1, xr_strlen(value), f);
            fclose(f);
            Msg("* [DA_PORT] анимации: %s (применится после перезапуска игры)", value);
        }
        else
            Msg("! [DA_PORT] анимации: не удалось записать %s", marker);

        if (1 == ps_da_animations && !da_animations_archive_present())
            Msg("! [DA_PORT] анимации: включены, но архива da_animations.xdb0 в database НЕТ - "
                "движений не будет");
    }
};

// [DA_PORT] Поднять состояние модуля анимаций из признака — по тем же соображениям, что и сезон:
// файловая система уже приняла решение по этому файлу, и меню обязано показывать её решение, а не
// своё. Признака нет — модуль включён (архив в этом случае монтируется).
static void da_animations_read_marker()
{
    string_path marker;
    FS.update_path(marker, "$arch_dir$", "da_animations.txt");
    ...
    if (FILE* f = fopen(marker, "rb"))
    {
        ...
        fclose(f);
        buf[got] = 0;
        ps_da_animations = (nullptr != strstr(buf, "off")) ? 0 : 1;
    }
    else
        ps_da_animations = 1;
    ...
    if (1 == ps_da_animations && !da_animations_archive_present())
        ...
        Msg("* [DA_PORT] анимации: %s", (0 == ps_da_animations) ? "выключены" : "включены");
}
...
// [DA_PORT] Снятие джиттера при восстановлении позиции в освещении. 0 - как было, 1..4 - варианты
// знака. Разбор - в shaders/r3/common_functions.h у gbuffer_load_data.
//
// Зачем перебор знаков, а не одно значение. Такую поправку уже пробовали 29.07 и откатили 30.07 как
// УХУДШАЮЩУЮ. Но при неверном знаке поправка не снимает ошибку, а удваивает - и выглядит это ровно
// как "поправка и была причиной". Отличить одно от другого можно только перебором, поэтому здесь
// четыре варианта, а не выключатель.
//
// 1 = (-x, -y), 2 = (-x, +y), 3 = (+x, -y), 4 = (+x, +y).
ENGINE_API int ps_r__taa_pos_fix = 1;
...
void CCC_Register()
{
    ...
    CMD4(CCC_DaDebugInteger, "r__motion_vectors", &ps_r__motion_vectors, 0, 9); // 5 = eye-space depth, 6 = [DA_PORT] G-buffer normal
    ...
    CMD4(CCC_DaDebugInteger, "r__water_depth", &ps_da_water_depth, 0, 1);
    CMD4(CCC_Float, "r__detail_mipbias", &ps_da_detail_mipbias, -2.f, 4.f);
    CMD4(CCC_Float, "r__detail_normal_fade", &ps_da_detail_nfade, 0.f, 500.f); // [DA_PORT]
    CMD4(CCC_Float, "r__macro_var", &ps_da_macro_var, 0.f, 1.f);                  // [DA_PORT]
    CMD4(CCC_Float, "r__macro_var_scale", &ps_da_macro_var_scale, 2.f, 64.f);     // [DA_PORT]
    CMD4(CCC_Float, "r__macro_var_start", &ps_da_macro_var_start, 0.f, 300.f);    // [DA_PORT]
    CMD4(CCC_Float, "r__macro_var_end", &ps_da_macro_var_end, 10.f, 1000.f);      // [DA_PORT]
    CMD4(CCC_Float, "r__macro_tint", &ps_da_macro_tint, 0.f, 1.f);              // [DA_PORT]
    CMD4(CCC_Float, "r__macro_relief", &ps_da_macro_relief, 0.f, 2.f);          // [DA_PORT]
    CMD4(CCC_Float, "r__terrain_blend", &ps_da_terrain_blend, 0.f, 1.f);        // [DA_PORT]
    CMD4(CCC_Float, "r__mask_jitter", &ps_da_mask_jitter, 0.f, 0.05f);          // [DA_PORT]
    CMD4(CCC_Float, "r__macro_detail", &ps_da_macro_detail, 0.f, 1.f);          // [DA_PORT]
    CMD4(CCC_Float, "r__lod_hemi", &ps_da_lod_hemi, 0.f, 2.f);                    // [DA_PORT]
    CMD4(CCC_Float, "r__lod_sat", &ps_da_lod_sat, 0.f, 3.f);                      // [DA_PORT]
    CMD4(CCC_Float, "r__lod_bright", &ps_da_lod_bright, 0.2f, 2.f);               // [DA_PORT]
    CMD4(CCC_Float, "r__alpha_hash", &ps_da_alpha_hash, 0.f, 1.f);
    CMD4(CCC_DaDebugInteger, "r__frame_diff", &ps_da_frame_diff, 0, 2);
    CMD4(CCC_Float, "r__frame_diff_gain", &ps_da_frame_diff_gain, 1.f, 200.f);
    {
    ...
    CMD4(CCC_DaDebugInteger, "r__light_probe", &ps_da_light_probe, 0, 7);      // [DA_PORT]
    // [DA_PORT] Отладочная — НЕ сохраняется в user.ltx. Напарник r__light_probe заведён верно,
    // а эта по недосмотру попала в сохраняемый класс: осталась бы у игрока навсегда.
    CMD4(CCC_DaDebugFloat, "r__light_probe_gain", &ps_da_light_probe_gain, 1.f, 64.f); // [DA_PORT]
    CMD4(CCC_Float, "r__fog", &ps_da_fog, 0.f, 1.f);                             // [DA_PORT]
    CMD4(CCC_Float, "r__shafts_sky", &ps_da_shafts_sky, 0.f, 1.f);               // [DA_PORT]
    CMD4(CCC_Float, "r__sss", &ps_da_sss, 0.f, 1.f);                             // [DA_PORT]
    CMD4(CCC_Float, "r__sss_len", &ps_da_sss_len, 0.05f, 2.f);                   // [DA_PORT]
    CMD4(CCC_Float, "r__sss_thick", &ps_da_sss_thick, 0.05f, 4.f);               // [DA_PORT]
    CMD4(CCC_Float, "r__sss_steps", &ps_da_sss_steps, 2.f, 32.f);                // [DA_PORT]
    CMD4(CCC_Float, "r__fog_sky", &ps_da_fog_sky, 0.f, 1.f);                     // [DA_PORT]
    CMD4(CCC_Float, "r__fog_sky_mip", &ps_da_fog_sky_mip, 0.f, 10.f);            // [DA_PORT]
    CMD4(CCC_Float, "r__fog_sky_flat", &ps_da_fog_sky_flat, 0.f, 1.f);            // [DA_PORT]
    CMD4(CCC_Float, "r__fog_height", &ps_da_fog_height, 0.f, 4.f);               // [DA_PORT]
    CMD4(CCC_Float, "r__fog_height_falloff", &ps_da_fog_height_falloff, 0.001f, 0.5f); // [DA_PORT]
    CMD4(CCC_Float, "r__fog_height_base", &ps_da_fog_height_base, -500.f, 500.f); // [DA_PORT]
    CMD4(CCC_Float, "r__fog_dist", &ps_da_fog_dist, 0.1f, 8.f);                  // [DA_PORT]
    CMD4(CCC_Float, "r__fog_follow_vis", &ps_da_fog_follow_vis, 0.f, 2.f);      // [DA_PORT]
    CMD4(CCC_Float, "r__fog_max", &ps_da_fog_max, 0.f, 1.f);                     // [DA_PORT]
    ...
    da_animations_read_marker();
    CMD3(CCC_DaAnimations, "da_animations", &ps_da_animations, qda_animations_token); // [DA_PORT]
    ...
    CMD4(CCC_Integer, "r__dlss_preset", &ps_r__dlss_preset, 0, 15); // [DA_PORT]
    ...
    CMD4(CCC_Integer, "r__taa_pos_fix", &ps_r__taa_pos_fix, 0, 4); // [DA_PORT]
