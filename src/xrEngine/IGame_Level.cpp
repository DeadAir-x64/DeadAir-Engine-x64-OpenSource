...
void IGame_Level::net_Stop()
{
    ...
    // [DA_PORT] Список параллельных задач обнуляем ДО уничтожения объектов.
    //
    // Задачи в нём живут ровно один кадр и снимаются в конце ProcessFrame, но здесь кадров больше
    // не будет: шесть обновлений выше успели набить список делегатами на объекты, которые следующей
    // строкой перестанут существовать. Исполнять их некому и незачем, а вот попытка исполнить —
    // обращение по освобождённой памяти.
    //
    // Это страховка сверх снятия делегатов в net_Destroy: та работает по одному объекту и требует,
    // чтобы каждый за собой убрал, а эта закрывает список целиком, включая тех, кто уничтожается
    // не через net_Destroy.
