...
void CRenderDevice::ProcessFrame()
{
    ...
    const auto& processSeqParallel = TaskScheduler->AddTask([this, perf]
    {
        ...
        for (u32 pit = 0; pit < count && pit < seqParallel.size(); pit++)
        {
            ...
            //
            // [DA_PORT] Обратная сторона такого ключа: объекты приходят и уходят, а записи о них
            // остаются, и карта растёт всю сессию — удаления не было ни одного. Ставим потолок:
            // сверх него новые ключи просто не заводим, уже накопленное продолжает считаться.
            // Порядок величины задач в кадре — сотни, так что предел до статистики не дотягивается,
            // но и бесконечного роста при включённом стороже больше нет.
            if (g_da_seq_stats.size() >= 20000 && g_da_seq_stats.find(key) == g_da_seq_stats.end())
                continue;
